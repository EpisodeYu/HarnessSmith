# 04 · `forge add` / 增量再生成 + 模板升级（结构性护城河详设）

> 本文是一份**面向 v1+ 的详细设计**，不改任何已定全局决策；落地前关键设计点仍需人签（见 §8）。
> 由 `03-feature-landscape-and-proposals.md §6 D-1` 升格而来——它被判定为**唯一的结构性护城河**：竞品（托管平台 / 你 import 的框架）结构上做不到，因为它们都不交付一份你拥有的代码。
> 口径基准：`01-project-plan.md §4`（结构轴 / 行为轴、天花板 / 地板）、`02-development/00-overview.md §2`（路线图，本特性在 **Slice 13+ / v1+**，人 2026-06-07 排期；仍为头号差异化，先做 Phase 1 安全子集）。
...
|----------|--------------------------|
| 无代码 / 托管平台（Vellum、MindStudio…） | 你没有代码可被 `add`——能力藏在平台后端，加功能 = 改它的配置，仍是锁定 |
| 你 import 的框架（LangGraph、Agents SDK…） | 框架的"加能力" = `pip install` 一个包，**不改你的循环**；你的编排逻辑要么在框架里、要么你自己手写——没有"把生成的代码增量演进"这件事 |
| 静态脚手架（cookiecutter 类） | 一次性吐模板，**没有回头路**；改了就回不去，升级 = 重抄 |

**只有"生成你拥有的代码 + 留着 spec 快照"的形态，才能既给你完整的代码所有权、又能在事后安全地增量长出新能力。** 这把 `01 §4` 的"结构轴 / 能力天花板"从一次性变成**可演进**，是把定位做成产品杠杆的最强一手。

## 3. 三种操作，必须精确区分

| 操作 | 语义 | 改什么 | 现状 |
|------|------|--------|------|
| **`add`** | 结构轴**新增**一段缺席的能力代码（web / mcp / skills / 一个内置范式） | 新增文件 + 受控插入扩展点 + 回写 `harness.spec.yaml` + 重跑 `uv lock` | ❌ 未做（本文主体） |
| **`upgrade`** | 把产物的模板版本**演进**到新的生成器版本（bugfix / 新默认） | 对已生成文件做 codemod / 3-way merge，靠 `harness.spec.yaml` 的 `version` 定位基线 | ❌ 未做（最难，Phase 3） |
| **`regenerate`** | 用（可能改过的）spec **重生成**整仓 | 全量覆盖 | ⚠️ 仅有"重跑警告不覆盖"；安全的整仓覆盖未做 |
...
整仓一次性渲染是干净的；**增量渲染进可能被编辑过的文件**才是真问题。三条要守：
...
**扩展点（注册表）就是天然的"缝"。** 产物的可扩展处都是注册表 + 装饰器：`tools`（`@tool`）、`paradigms`（`@register_paradigm`）、`context`（`@register_strategy`/`@register_condition`）。新增项多数**只是新增一个文件**——但要让它"被发现"，当前实现需要一行 import：

```python
# paradigms/__init__.py（节选）——内置范式靠显式 import 触发注册，且是 Jinja 条件块：
from . import agent                              # 总在
{% raw %}{% if "plan" in spec.paradigms %}{% endraw %}from . import plan
{% raw %}{% if "ask" in spec.paradigms %}{% endraw %}from . import ask
```

于是"加一个内置 `plan` 范式"**不是纯新增**——要在 `__init__.py` 的 import 块插一行。这把 `add` 分成两档难度，决定了分阶段策略（§6）：

- **A. 纯新增、零改已有文件**：能力的"接线"发生在**运行期 `config.yaml`**（不是代码）。例：开一个 MCP server（写 `config.yaml mcp.servers`）、加一个 skill 目录、加一个工具进 `tools.enabled`。这些 `forge add` 今天就能安全做。
- **B. 需受控插入到代码扩展点**：能力要落**新代码文件 + 一行接线**（新范式文件 + `__init__` import；开 web 要新增 `interfaces/web.py` 且 `pyproject` 加 extra）。需要"锚点插入"或改造发现机制。

> **设计岔路（值得单独决策）**：若把范式注册表从"显式 import"改成"**扫描目录自动发现**"，则 `forge add paradigm` / 用户自加范式都退化成**纯新增一个文件**（A 档）。代价是牺牲一点"显式即薄"的可读性（`CLAUDE.md §2`）。**建议：仅对范式这一个高频扩展点评估自动发现，其余维持显式。** 见 §8 决策②。
...
只做"不碰任何已有代码文件"的 add：
- **运行期能力**：`forge add mcp-server <name>`（写 `config.yaml`，复用 Slice 6 catalog 预填逻辑）、`forge add skill <name>`（新建 `skills/<name>/SKILL.md`）、把工具加进 `tools.enabled`。**零代码改动、最低风险**。
...
做"新增文件 + 一处接线"的 add：`forge add web`、`forge add mcp`（开 `mcp.py` + extra）、`forge add paradigm plan`、`forge add skills`。
...
把已生成产物升到新生成器版本（拿新 bugfix / 新默认），靠三方合并：
- **基线** = 用旧版生成器 + 产物的 `harness.spec.yaml` 重新渲染得到的"原始产物"（即用户编辑前的样子）。
- **三方** = (基线, 新版渲染, 用户当前文件) → 标准 3-way merge。
- 干净 hunk 自动并；冲突落 `git` 冲突标记交用户。
- **诚实**：3-way merge 进被深改的核心循环必然有冲突；`upgrade` 的承诺只能是"把能自动并的并掉、其余清晰标出"，不是"无痛升级"。**优先保证 Phase 1/2，`upgrade` 可长期停在"仅升级未被用户改过的文件 + 其余报告差异"的弱版本。**
...
1. **范围与排期**：`forge add` 是否作为 v1+ 的**头号差异化立项**（投入最大、护城河最深）？先只做 **Phase 1 安全子集**（低风险、覆盖高频诉求）验证价值，再决定 Phase 2/3？
2. **范式发现机制**：是否把 `paradigms/` 由"显式 import"改为"目录自动发现"以让 `forge add paradigm` 纯新增？（牺牲一点显式可读性，换增量友好——`CLAUDE.md §2` 薄/可读权衡）
3. **`add` 是否允许改 `pyproject.toml`**：开 web/mcp 要加 optional-deps——确认走结构化 TOML 编辑（非正则）、且只动 `[project.optional-dependencies]`。
4. **clean-tree 前置**：默认要求 git working tree clean、否则拒绝（`--force` 跳过）——确认这是可接受的 UX 约束（产物本就 `git init`，契合）。
5. **`upgrade` 的承诺边界**：明确对外只承诺"自动并可并的、清晰标冲突"，不承诺无痛升级，避免预期落差。
...
- **密钥不入 git/spec/trace**：`add` 写的是结构（代码 / `config.yaml` 的 env 名 / spec），**绝不写 key 值**，沿用既有纪律。✅

## 10. 风险与未决

- **3-way merge 进深改核心循环**：本特性最硬的部分；缓解 = Phase 化、先 Phase 1、`upgrade` 给弱承诺。
- **锚点注释污染"薄/可读"卖点**：锚点要少而克制（只在真正需要接线的极少数文件），否则产物读起来像被工具占领。
- **spec 快照与用户手改 `config.yaml` 漂移**：`add` 以 spec 为结构真相，但用户可能直接改了 `config.yaml`（行为轴）；`add` 只动结构、不覆盖行为值，需在合并 `config.yaml` 时只增不改已有键。
