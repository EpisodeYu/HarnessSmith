...
- **原生 function-calling 循环**:用 API 的 `tool_calls`(TAO/ReAct 语义),含停止条件(模型给出最终答案 / 取消 / 按 LLM cost 限额,见预算行)/错误处理;**走 Chat Completions API**(见 §6)。(注:2026-06-11 起不再内置 `max_steps` 步数护栏——防工具无限循环归工具策略,与 cost 限额无关。)
- **工具注册表**:装饰器注册,新增 tool = 加函数 + 注册,不改循环;含 1–2 个内置安全示例工具。
- **CLI(Typer)**:`run` 一问一答。
- **JSONL trace + token/成本计数**。
- **按 LLM cost 限额停止**(2026-06-11 全面重做,人定向,推翻原 per-run 可组合条件):产物 `harness/usage.py` 的 `UsageLedger` 按 **profile name** 把 token 用量**持久累计**到 `.harness/usage.json`(跨 run;口径=主 generation + compaction,经 `Trace.add_usage` 记账;title/memory 不计),**cost 由各 profile 当前单价派生**;每 profile `cost_limit`(0=不限)到达即**调用前阻止该 LLM 并优雅停本轮(`stop_reason="llm_budget"`)+ 提醒**(block_stop,不自动 fallback)。单价+限额=运行期活旋钮(Web Budget 页唯一入口 / CLI `usage`);**清空手动**。**抛弃** `max_steps`/`max_seconds`/`max_tokens`/`max_cost` + 条件注册表 + `BudgetTracker`(防工具无限循环归工具策略,与限额无关)。天/周/月周期配额仍 v1+(复用同账本)。
...
> **v1 排期更新(人 2026-06-07 定向)**:MVP(Slice 0–7)收口后,对标 2026 事实标准 harness(Claude Code/Codex/Cursor/Cline 等)发现三个"已成标配、本项目尚缺、且薄+不触红线+强契合 own-your-code"的缺口,升格为 **v1 必做切片**:**Slice 8 会话持久化(resume/`--continue`)/ ~~Slice 9 Checkpoints(危险操作前 git 快照 + 回滚)~~(2026-06-08 评估撤销;槽位重定义为停止/继续/重问会话时间旅行,见 `02-development/10-slice-9-stop-resume-reask.md`)/ Slice 10 工具调用 HITL 交互确认**。同时把 **跨会话全局记忆** 从原 v2 上移 **v1**。**2026-06-07(人定向排期)** 进一步排定 v1 后续顺序:**Slice 8 会话持久化 → Slice 8B 跨会话记忆**(紧接、共用会话落盘基础设施,沿用 Slice 6B 插入式编号)**→ Slice 9 停止/继续/重问**(会话级时间旅行,槽位由撤销的文件级 Checkpoints 重定义,详见 `02-development/10-slice-9-stop-resume-reask.md`)**→ Slice 10 HITL 交互确认**(`before_tool` 交互确认,独立做)**→ Slice 11 MCP 健康/管理**(`mcp status`/`doctor` + 面板增删/热重连,升格自 v1+)**→ Slice 12 原生 Anthropic 双规范 + 推理流式**(§6.4 方案已写出待人签,见 [`05-llm-dual-spec-anthropic.md`](./05-llm-dual-spec-anthropic.md))**→ Slice 14+** v1+ backlog(`forge add` / eval / presets / multi-agent / RAG 等)。对标分析见 [`03-feature-landscape-and-proposals.md`](./03-feature-landscape-and-proposals.md);头号差异化 **`forge add` / 增量再生成 + 模板升级** 详设见 [`04-forge-add-incremental-regeneration.md`](./04-forge-add-incremental-regeneration.md)。门禁与口径以 [`02-development/00-overview.md §2`](./02-development/00-overview.md) 为准。

- Web `/config` 运行期热重载面板、密钥只写不回显面板、**联网 MCP registry / `forge add` 增量接 server**(Slice 14+)、完整 HITL Web 交互时序、**工具调用 HITL 确认**(allow/reject/always-allow;**今天已可经 `before_tool` hook `raise`-veto 自实现**;**2026-06-07 升格为 v1 必做 Slice 10**——内置 `ConfirmHooks`、护栏非保证、非交互/Web 默认拒绝;据此把 shell 默认开 = 改 §6 口径需人签)、**MCP 状态自检/健康 + 面板增删·热重连**(probe server 连通 + 工具计数 + 不可达标红,CLI `mcp status`/`doctor` + `/config` 健康视图,复用 `McpManager.errors`/`discovered`;**2026-06-07 升格为 Slice 11**)、RAG 最小 ingest 闭环 + sqlite-vec、**原生 Anthropic 双规范 + 推理流式**(2026-06-07 升格为 Slice 12,§6.4 方案待签,见 `05-llm-dual-spec-anthropic.md`)、keyring 密钥后端、context offload(大输出落盘)。(注:**MCP 远程 HTTP/SSE 传输已于 2026-06-03 提前进 L2**,见 L2;registry / 增量接入仍 Slice 14+。)
...
- **周期预算**(候选,v1+):**按 LLM 持久 cost 账本 + 限额**已于 2026-06-11 落地为默认能力(`harness/usage.py`,`.harness/usage.json` 跨 run 累计、cost 由单价派生、`cost_limit` block_stop;见 §3 L1 与 `00-overview` 决策表"预算"行)。**仍排 v1+ 的只剩"周期"那层**:天/周/月配额 + 自动按期重置/分桶(本地 JSON 已是持久账本,多进程/Web 再换 sqlite+锁),复用同一 `usage.json`。
...
- `src/<pkg>/interfaces/tui.py`(L2,Slice 13)— 全屏终端 agent(Textual):默认连续对话 + 全字段 config 面板;`interfaces.tui` 门控,关掉零痕迹。
