...
>
> **实现说明(与计划的细化)**:① **Web 依赖落位**取方案 A——`web=true` 时 `fastapi`/`uvicorn` 直接进 `dependencies`(`uv sync`/Docker/`smoke_check` 零改动即装上),`web=false` 时整段不渲染;测试依赖 `httpx`(`fastapi.testclient` 需要)同样条件进 `[dependency-groups] dev`。② **`/config` 持久化**:本片初版做"面板改 → 进程内 `app.state.config` 生效、**不回写 `config.yaml`**"(理由见 §4)。**实现说明(2026-06-09 改判,人签字):改为回写 `config.yaml`**——用户反馈"每次重启 serve 都要重配"太反常识(尤其一键 wizard 产物 model 留空、key 落 `.env` 而 model 不落盘,重启后 chat 又被 gate)。当年"避免 dump 丢注释"的顾虑用 **`ruamel.yaml` 注释保留 round-trip** 解决:`save_config` 读现有文件 → deep-merge 可编辑字段 → 写回,块级/字段注释与"被注释的可选项菜单"保留(列表 llms/tools 被整体替换时其元素行内注释会丢,可接受);`ruamel.yaml` 仅随 `interfaces.web` 进产物依赖(默认薄核心不含)。详见 §4 软确认结论。③ **SSE = token 级流式 + 进度事件,且可选**(人 2026-06 追加要求,UX 关键):给 `LLMClient` 增 `stream(messages, tools, on_delta)`(Chat Completions `stream=True`,累积 content/tool_calls/usage 并逐 token 回调;仍是 Chat Completions,**未切 Responses / 未换 SDK**,不触发 `CLAUDE.md §6.4`);`loop.run()` 增可选 `on_delta`,有则走 `stream` 否则 `complete`(`loop.py` 仅 +4 行,实测 180 行仍在 150–300 薄区间)。流式开关在调用方:web `/chat?stream=`(默认开)+ 页面复选框、CLI `run --stream/--no-stream`(默认关,保留原行为)、库级传 `on_delta`。mock 端 `stream()` 复用 `complete()` 逐词发 token,离线可测。④ **会话页流式开关移除 + 上下文占用圆圈(2026-06-11,人定向)**:页面不再露出"流式"复选框——产物 Web 始终 token 级流式;服务端 `/chat?stream=` 参数保留(`?stream=false` 仍供测试 / 程序化调用,产物 `test_web.py` 用之)。原开关位置改为**上下文占用圆圈**:`event: run` 携带 `context_window`(generation profile 的窗口,未设则不算占比)、每个 `event: step` 携带真实 `prompt_tokens`,环形按占比填充(>85% 转琥珀、>95% 转红,对齐 `window_pct` 默认 0.85),点击弹出"已用 / 窗口 token(占比)"。纯前端 + 两个 SSE 字段,不动 harness 核心。同批 `/config` 上下文卡片微调:`combine` 下拉移到触发条件下方并更名"触发组合方式"、触发条件列表 `window_pct` 置顶、`keep_last_turns` 中文标签作"完整保留最近轮数"。
> ⑤ **系统页:主题 + 流式开关 + 配置导入/导出 + 语言(2026-06-11,人定向)**:产物 Web 增"系统"能力,纯属现有 `interfaces.web` 接口内的 UX 增强,**未改 `HarnessSpec`、关 Web 时零痕迹、零新增依赖**(导入/导出复用已随 web 进产物的 `yaml`/`Config`)。四件:
> - **主题切换**(侧栏 `Config` 按钮**上方**,与 `Config` 行同构对齐:左侧 🎨 图标 + "Theme" 圆角框 `flex-1`、右侧浅/深切换):两档 `浅色(默认)/ 深色`(**人定向:去掉"跟随系统"、默认浅色**),纯前端 localStorage(`hf_theme`,默认/`light` 不加 `dark`、`dark` 加),`<head>` 内联脚本首屏即应用防闪烁,`<html>` 切 `dark` class。深色用**受控 CSS 覆盖**(`html.dark` 覆盖本页有限调色板,~45 行,非逐元素加 `dark:`,守"薄"+"surgical")。**`Config` 行视觉对齐**(人定向):`Config` 是按钮,原激活/hover 用 `bg-slate-100` 填充圆角框,与 `Theme` 纯文字标签不一致(深色 `#334155` 更明显)——改为**激活仅加粗(`font-medium`)、去掉填充框 + `cursor-pointer`**,两行左侧图标+文字 `flex-1` 严格对齐、无突兀色块。
> - **流式开关**(`system` 子标签,人定向:从聊天表单移到系统页):**Enable/Disable 两段式**(非勾选框)、**默认 Enable**,客户端偏好 `hf_stream`(`"0"`=关)驱动 `/chat?stream=`(服务端参数早已存在)。同时修了前任 commit(`1f9efd6` 移除聊天流式勾选框)遗留的 `gateChat` 仍引用已删的 `$("stream")` 的悬挂 bug。
> - **配置导入/导出**(`system` 子标签):`GET /system/config-export` 下载整份 `config.yaml`(有 `config_path` 取盘上原文保注释,否则序列化内存 config;**只含 env 名、永不含密钥真值**);`POST /system/config-import` 接 raw YAML/JSON → `Config.model_validate` 校验(非法不动运行中 config,返 400)→ 进程内替换 + 复用 POST /config 的重挂逻辑(debug log / 扩展 / 记忆工具 / MCP 重连)→ verbatim 写回 `config.yaml`(无 `config_path` 则只进进程、不落盘)。前端文件上传 + 文本粘贴,导入前 `confirm` 提示"替换当前配置"。**本地可信控制面**(同 MCP 编辑面,勿对公网暴露 `/config`/`/system`,纳入 Slice 14+ 发布隔离前提)。
> - **语言**选择**仅**在 `system` 子标签(`#sys-lang`;**侧栏选择器已移除**,避免与系统页重复——人定向),共享 `hf_lang` 状态。
> 门禁:产物 `test_web.py` 新增导出/导入(成功 + 非法被拒 + 无路径不落盘)+ index 断言(系统页/主题仅浅深/流式开关/`html.dark`/两端点),生成器 `test_generator.py` 加系统页/主题 web 侧断言;全功能产物(web+mcp+memory+skills)`uv sync && pytest` 绿、关记忆 web 足迹断言绿(`config-export`/`config-import` 文案避开 `memory`/`mcp` 子串以守零痕迹)、浏览器实测浅/深色 + 系统页。
...
- **进阶热重载**(watch 文件、多进程一致性)= v1+(`00-overview §2` Slice 14+ 行),本片只做"面板改 → 进程内生效"。
...
- ~~**v1+ 待办(2026-06-03 真实 LLM 验收发现)**:推理模型在 reasoning 阶段先沉默数秒才吐 content,流式"无反应等待"~~ **✅ 已解决(Slice 12,2026-06-10 思考通道 + 2026-06-11 Web 可视化升级)**:`event: thinking` 思考流 → 产物 Web 升级为四态可视化(发送即"思考中"动画占位 → reasoning 流入独立折叠框 → 答案首 token 折叠为"已思考 N 秒" → 工具调用合一折叠块),历史重开经 `renderHistory` 还原;reasoning 持久化(工具轮 `reasoning_content` / 最终轮显示专用 `reasoning_display`)。详见 `05-llm-dual-spec-anthropic.md` §5 实现说明 + §8 补遗 3。
- **v1+ 待办(2026-06-05 部署拓扑讨论发现):`/config` 须与公开面隔离**。现状:`/chat` 与 `/config`(读 + 改运行期行为性配置)挂在**同一 FastAPI app、同端口、无鉴权**(`web.py` `create_app`)。在"**管理员托管 + 接口发布**"拓扑(`01 §4` 拓扑②,实际更常见)下,把 web 接口发布给终端用户 = 用户也能 POST `/config` 改掉管理员配好的 prompt / 预算 / tool allowlist——**配置即接口的安全面被击穿**。此拓扑下运行期配置本应安全(用户在网络接口另一侧、够不着 `config.yaml`),但前提是**管理面与公开面隔离**。**v1+ 落地候选**:① `/config` 加鉴权(token / 管理员凭证);② `/config` 仅绑 `127.0.0.1`、`/chat` 对外;③ **生成期开关**让发布实例不渲染 / 只读 `/config`(即 `01 §4` "`/config` 存在性 = 结构轴开关",会动 `HarnessSpec` → 触发 `CLAUDE.md §6.1`)。会动代码、③ 还动 spec,实现前请人签字。已登记到 `00-overview §2` Slice 14+ backlog(**且为 Slice 11 面板改 `mcp` 的前提**)。
- **软确认结论(Agent 已自主决定,`CLAUDE.md §5.3`;非阻塞,可一句话改判)**:
  ...
  - **配置面板"未保存"守卫(2026-06-07 UX 细化,Agent 自主)**:面板编辑仅在 DOM、点 Save 才 `POST /config` 生效,用户常忘按 Save 致刚配的 LLM profile 丢失。落地为**未保存提醒**而非自动保存(自动保存会与服务端 Pydantic 校验/重渲染/即时生效心智三处打架,且违反"薄"):脏标记(任一编辑亮"未保存"+高亮 Save)、切走 Chat / 刷新前 `confirm`、切回 Config 时 `!dirty` 才 `loadConfig()`(修根因——原先每次切回都无脑重载覆盖未保存编辑)。纯前端 `web_index.html`,产物 HTML 自带断言(`test_web.py::test_index_has_unsaved_config_guard`)。
    - **实现说明(2026-06-09 修 bug,Agent 自主)**:脏标记原由 `#view-config` 上的 `input`/`change` 监听直接 `setDirty(true)`、Save 后 `setDirty(false)` 关掉。但 `renderConfig()` 重建面板时 `innerHTML=""` 拆掉**当前聚焦**的字段,浏览器对被拆字段补发的 `change`/`blur` 是**异步**的,会落在 Save 的 `setDirty(false)` **之后**,把"未保存"又点亮——表现为点保存后"保存/重载/未保存/已保存"四者并存且 Save 仍高亮(尤见于点按钮不夺焦的浏览器,如 Firefox/Safari)。改为**比较法**判脏:每次 load/Save 后用 `configSnapshot()`(= `collectConfig()` + rule 行的路径/正文)记下 `savedSnapshot`,任一编辑都 `refreshDirty()`(当前快照 ≠ 基线才算脏)。重建拆字段补发的杂散事件不改变序列化形态 → 重算仍等于基线 → 不会误标脏;顺带把"改回原值"也正确判为已保存。门禁:产物 142 测试 + mock function-calling 绿、生成器 122 绿、浏览器实测(编辑→保存→注入杂散事件仍保持"已保存")。
