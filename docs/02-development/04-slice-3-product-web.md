...
> - **主题切换**(侧栏 `Config` 按钮**上方**,与 `Config` 行同构对齐:左侧 🎨 图标 + "Theme" 圆角框 `flex-1`、右侧浅/深切换**正上方对齐语言下拉**):两档 `浅色(默认)/ 深色`(**人定向:去掉"跟随系统"、默认浅色**),纯前端 localStorage(`hf_theme`,默认/`light` 不加 `dark`、`dark` 加),`<head>` 内联脚本首屏即应用防闪烁,`<html>` 切 `dark` class。深色用**受控 CSS 覆盖**(`html.dark` 覆盖本页有限调色板,~45 行,非逐元素加 `dark:`,守"薄"+"surgical")。
...
> - **语言**整合进 `system` 子标签(侧栏原选择器保留,共享 `hf_lang` 状态)。
