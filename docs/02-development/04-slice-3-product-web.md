...
> ⑤ **系统页:主题 Light/Dark + 配置导入/导出 + 语言(2026-06-11,人定向)**:产物 Web 增"系统"能力,纯属现有 `interfaces.web` 接口内的 UX 增强,**未改 `HarnessSpec`、关 Web 时零痕迹、零新增依赖**(导入/导出复用已随 web 进产物的 `yaml`/`Config`)。三件:
> - **主题切换**(侧栏 `Config` 按钮**上方**,人定向位置):三档 `跟随系统 / 浅色 / 深色`,纯前端 localStorage(`hf_theme`),`<head>` 内联脚本首屏即应用防闪烁,`<html>` 切 `dark` class。深色用**受控 CSS 覆盖**(`html.dark` 覆盖本页有限调色板,~45 行,非逐元素加 `dark:`,守"薄"+"surgical")。
> - **配置导入/导出**(Config 内新增 `system` 子标签):`GET /system/config-export` 下载整份 `config.yaml`(有 `config_path` 取盘上原文保注释,否则序列化内存 config;**只含 env 名、永不含密钥真值**);`POST /system/config-import` 接 raw YAML/JSON → `Config.model_validate` 校验(非法不动运行中 config,返 400)→ 进程内替换 + 复用 POST /config 的重挂逻辑(debug log / 扩展 / 记忆工具 / MCP 重连)→ verbatim 写回 `config.yaml`(无 `config_path` 则只进进程、不落盘)。前端文件上传 + 文本粘贴,导入前 `confirm` 提示"替换当前配置"。**本地可信控制面**(同 MCP 编辑面,勿对公网暴露 `/config`/`/system`,纳入 Slice 14+ 发布隔离前提)。
> - **语言**整合进 `system` 子标签(侧栏原选择器保留,共享 `hf_lang` 状态)。
> 门禁:产物 `test_web.py` 新增导出/导入(成功 + 非法被拒 + 无路径不落盘)+ index 断言(系统页/主题/`html.dark`/两端点),生成器 `test_generator.py` 加系统页/主题 web 侧断言;全功能产物(web+mcp+memory+skills)`uv sync && pytest` 225 绿、关记忆 web 足迹断言绿(`config-export`/`config-import` 文案避开 `memory`/`mcp` 子串以守零痕迹)、浏览器实测浅/深色 + 系统页。
