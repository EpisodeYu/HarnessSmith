...
- **④ 侧栏会话重命名 / 删除**:每个 session 行带 ✎(重命名,`window.prompt`)/ 🗑(删除,`window.confirm`)。后端 `session.py` 加 `set_title`/`delete`;`web.py` 加 `PATCH /sessions/{id}`(改标题、保留消息,缺会话 404)/ `DELETE /sessions/{id}`(幂等)。`_path` 用 `Path(id).name` 收窄,**杜绝路径穿越**(id 来自 URL)。删当前会话即回到空草稿。
...
- **不做**:会话级分支/编辑历史、多用户会话隔离(单租户,`01 §4`)、会话级预算账本(周期预算持久化 = Slice 13+)。
