...
- **catalog 在向导 = 策展子集**：显示 fetch/web-search/git/desktop-commander（默认勾前三项、DC 在最后),隐藏 github（需 token）与 time（冷门）；catalog 本体不变（CLI `--mcp-server` 照常可用,含 uvx 备选 ddg-search)。默认搜索 server 由单引擎 bing-search 改为多引擎 web-search（详见 [`12-slice-11-mcp-management.md`](./12-slice-11-mcp-management.md) §1 后续增强）。
