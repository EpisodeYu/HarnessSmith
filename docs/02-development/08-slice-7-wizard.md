...
- **catalog 在向导 = 策展子集**：显示 fetch/bing-search/git/desktop-commander（默认勾前三项、DC 在最后),隐藏 github（需 token）与 time（冷门）；catalog 本体不变（CLI `--mcp-server` 照常可用,含墙外备选 ddg-search)。
