...
- **Windows 一键生成两处真机修复(2026-06-09,用户实测反馈驱动)**:① **目标目录斜杠混用**——前端 `index.html syncTarget()` 用硬编码 `"/"` 拼 `generate_base`+slug,Windows 的 `generate_base`(`str(Path)` 出反斜杠 `D:\…\generate`)拼成 `…\generate/slug`;改为**按 base 自身分隔符拼接**(含 `\` 用 `\`、否则 `/`)。② **`uv sync` 卡死不动**——`app.py _uv_sync` 跑产物 `uv sync` 时**无超时**且**原样继承 wizard 环境**:wizard 自身由 `uv run` 在 HarnessForge venv 内启动、导出 `VIRTUAL_ENV`(指向那个 venv),产物 sync 继承后会去动父进程**正在用**的 venv,Windows 下文件被锁→sync 永不结束、UI 一直转(墙内 PyPI / `python-build-standalone` 下载被卡也是同样无界 hang)。修法:新增 `_product_env()`**剥掉 `VIRTUAL_ENV`/`UV_PROJECT_ENVIRONMENT`**(保留 launcher 设的 `UV_DEFAULT_INDEX`/`UV_PYTHON_PREFERENCE` 镜像)供产物 `uv sync`/`uv run` 用;`_uv_sync` 回 `(code, log_tail)`,`_run_launch` 把 `.setup.log` 末几行写进 `job.error`——失败有诊断。
- **`uv sync` 慢=像卡死的进度透明化(2026-06-09 续,用户反馈"等了相当久最后成功了")**:用户那次其实是**慢而非真死**(墙内下包/CPython 慢),所以原 `_UV_SYNC_TIMEOUT=600s` 反而有**误杀慢成功安装**的副作用。改:① 超时**放宽到 1800s** 且明确定位为"只兜底真·wedged 进程、不杀慢装"的安全网(`subprocess.run` 绝对墙钟超时,超时杀子进程回 124);② **实时进度**——`_run_launch` 把 `job["setup_log"]` 路径挂到 job,`GET /generate/status` 每次轮询时**现读 `.setup.log` 末 4 行**作 `log_tail` 返回(不改存储的 job);前端 `renderProgress` 在 sync 步**运行中**显示 uv 最新输出行 + **已用秒数** + 一句"首次安装会下若干包、慢网可能要几分钟"提示。**百分比有意不做**:uv 输出重定向到文件(非 TTY)时不发干净的百分比,硬解析脆;用"最新日志行 + 秒表 + 提示"表达"在动"即可。测试守卫:`_product_env` 剥父 venv 留镜像、status 端点在 `setup_log` 存在时回 `log_tail`、`_run_launch` 在 sync 失败时标 `sync=error`+录日志路径/尾且不进 serve。
- **镜像源自动兜底:探测官方 PyPI、不可达自动用清华(2026-06-09 续,人选"全部入口")**:在原"装 uv 时的 `[2]` 手动镜像菜单"之外,给**所有 `uv run`/`uv sync` 入口**加**自动**镜像选择,免菜单。机制统一:**HEAD 探测 `https://pypi.org/simple/`**(短超时),拿到任何响应(含 4xx/5xx)= 可达→用官方;连不上/超时(墙内被封)= 不可达→本次临时设 `UV_DEFAULT_INDEX=<清华源>`。**已显式设任一 index 环境变量(`UV_DEFAULT_INDEX`/`UV_INDEX_URL`/`UV_INDEX`/`PIP_INDEX_URL`)则一律不覆盖**(含 `[2]` 路径设的)。覆盖三层:① 向导产物 `uv sync`/`serve`——`app.py` 新增 `_pypi_reachable()`(urllib HEAD;HTTPError=可达、URLError/OSError=不可达)+ 进程级缓存的 `_auto_index()`,并入 `_product_env()`(剥父 venv 后,若无显式 index 且不可达则填清华);② **生成器启动** `HarnessForge.{bat,sh}` 的 `:run`/uv-present 路径——`curl -sS -m 3 -I` 探测、仅在有 `curl` 时探(Win10 1803+ 自带),不可达设 `UV_DEFAULT_INDEX`;③ **产物自带启动脚本** `__launch_name__.{bat,sh}.j2` 同款。**只动 index(wheels)、不碰 managed-Python 下载**(那要第三方 GitHub 代理,踩供应链红线,不做;系统有合适 Python 时 uv 本就不下)。**局限**:HEAD 探测只判"通不通",判不出"通但被限速慢";"慢但能用"由实时进度 UI(上一条)兜。测试守卫:`test_wizard` 测 `_product_env` 在不可达填清华/可达不填/有显式 index 不覆盖、`_pypi_reachable` 分类响应;`test_generator` 测两个产物启动脚本含 `pypi.org/simple/` 探测 + `curl` + `unreachable` 提示 + 清华 fallback。
- **墙内(中国大陆)镜像兜底(2026-06-09,人选"加国内镜像兜底")**:winget / 官方安装器都从 **GitHub Releases** 下 uv 二进制,墙内常卡死(用户实测卡在 winget)。Step 4 装 uv 改为**选项式** `[1] 标准(winget/官方,走 GitHub)` / `[2] 国内镜像(pip + 清华源,需已装 Python)` / `[n] 不装`。选 ② 走**全程可信、零 GitHub、零第三方代理**的路径:`python -m pip install --user uv -i https://pypi.tuna.tsinghua.edu.cn/simple`(清华源,uv 在 PyPI 上),再用 `python -m uv run …` 跑(确认 `python -m uv` 受支持,免去找 `uv.exe`),并设 `UV_DEFAULT_INDEX=<清华源>`(依赖走清华)+ `UV_PYTHON_PREFERENCE=only-system`(用系统 Python,**避免 uv 再从 GitHub 下 `python-build-standalone`**)。**有意不自动跑任何第三方 GitHub 代理/镜像脚本**(供应链信任红线,`CLAUDE.md` IMPORTANT):墙内无 Python 时只给指引(装 python.org 的 Python〔墙内可达〕→ 再选 ②,或挂代理选 ①),不偷偷 pipe 第三方 installer。`.sh` 同款 `[1/2/n]`(官方 `install.sh` / pip 清华 + `python3 -m uv`)。`UV_INSTALLER_GITHUB_BASE_URL`(覆盖 uv 安装下载源)等代理式方案只在文档/指引提及、不硬编码(代理域名易变、信任面)。测试守卫:`.bat`/`.sh` 含清华源 + `UV_DEFAULT_INDEX`/`UV_PYTHON_PREFERENCE`/`only-system`/`-m uv run` + `Choose [1/2/n]`。**[1] 标准路径去掉 winget 自动安装**(2026-06-09 用户日志确诊):winget 会先更新自身源 CDN,墙内**卡很久后 `0x80072efd` 失败**,而其后的官方 `install.ps1` + GitHub 下载本身是通的(同一用户 uv/CPython/包都装上了)——故 [1] **直接跑官方安装器**,winget 仅留在手动指引(其源更新挂起是纯浪费时间的拦路虎)。
- **便携 Node 自举(2026-06-10,人定向"保留 DC 默认 + 启动脚本引导装 Node")**:`desktop-commander`(npx)需 Node,而 Windows 装机率低。**不打包 node 进仓库/exe**(违薄、生成器在 Linux 产不出 Windows 二进制);改为**仅当预填了 Node 系 server 时**(生成期 `generator._build_context` 计算 `mcp_needs_node` = 任一 prefill server `requires==node` 或 `command∈{npx,npm,node}`),在产物 `__launch_name__.{bat,sh}` 顶部(uv 自举之前)注入一段**复用 uv 引导风格**的 Node 自举:先 `where node && where npx` / `command -v` 探测,缺则给 `[1] 官方 nodejs.org / [2] 国内 npmmirror / [n] 跳过` 菜单,下**便携 Node(pin LTS `generator.NODE_LTS_VERSION=v22.11.0`)**到用户级目录(`%LOCALAPPDATA%\<slug>` / `~/.local/share/<slug>`)、解压后**本会话 prepend PATH**(产物及其 npx 子进程继承)。Windows 走 `.zip`(PowerShell `Invoke-WebRequest`+`Expand-Archive`)、POSIX 走 `.tar.gz`(`curl|tar`,`uname` 探 os/arch)。`.bat` 仍守"扁平 goto、无 `setlocal`/`call`"(用 `:hf_node_ok` 标签跳过,不引子程序);跳过/失败均不致命(该工具留 off,其余照跑)。npm 拉 DC 包的镜像由产物 `mcp.py` 的 `_mirror_env` 运行期注入(已实现),与此处下 node 二进制的镜像选择分工。`node_version`/`mcp_needs_node` 进渲染上下文。**向导一键启动也补 Node(2026-06-10 续,用户实测:一键跳转不走 `.bat`/`.sh`)**:向导"一键生成"的后台 job 直接 `uv run <slug> serve`(`app._launch_product`),**绕过启动脚本**,故脚本里的 Node 自举不触发→产物起来后 DC 报 `npx not found`。抽共享 `harnessforge/node_bootstrap.py`(`ensure_portable_node(slug, log=)` + `node_on_path()` + `NODE_LTS_VERSION`,`generator` 改为从此 import 常量)——脚本的 Python 孪生(脚本在 uv 之前跑、用不了 Python,故 shell 版仍保留)。`_run_launch` 在 `sync` 后、`serve` 前:job 步骤动态插入 `node`(仅 `needs_node` 时,由 `_new_job(needs_node)` 决定;`_run_launch` 从 `job["steps"]` 是否含 `node` 推断,故 `_spawn_launch` 3 参签名不变、老测试不破),`node_on_path()` 有则跳过、否则 `ensure_portable_node` 静默下便携 Node(探测 `nodejs.org` 不可达→npmmirror)并把 bin 目录经新参 `extra_path` prepend 进 `_launch_product` 的 PATH。用户级目录与脚本同处、二者共享。**serve/node 步也加实时日志+秒表**(人定向"避免觉得卡死"):`job["setup_log"]` 指针随步骤切到 `.node.log`/`.serve.log`,`GET /generate/status` 照常 tail;前端 `renderProgress`/`trackJob` 由"仅 sync"泛化为**任一 running 步**显示 log 尾 + 计时(running 步切换即重置秒表),新增 `step_node`/`node_hint`/`serve_hint` i18n。测试:`tests/test_node_bootstrap.py`(平台 tag/跳过/下载解压布局/镜像回退)、`test_wizard.py::test_run_launch_provisions_portable_node_for_node_servers` / `test_run_launch_skips_node_download_when_node_present`;真机已验 npmmirror 拉 `v22.11.0` 解压布局 = `node-v22.11.0-<os>-<arch>/bin/node`、`npx` 可用。测试守卫:`test_generator.py::test_launch_node_bootstrap_only_when_a_node_server_is_prefilled`(DC 预填→两脚本含 Node 菜单/`v22.11.0`/双镜像、`.bat` 仍无 `setlocal`/`call :`;仅 uvx 预填→无 Node 逻辑);真机实测从 npmmirror 拉 `v22.11.0` 解压布局 = `node-v22.11.0-<os>-<arch>/bin/node`、`npx` 可用。
- **取数(源+代理)自动解析 + 预热 + 可读报错(2026-06-10,接 Node 自举的后续:Node 装好但 DC 仍连不上)**:真机(企业代理)实测——便携 Node 已装好,但 `desktop-commander` 仍黄灯转很久后 `unhandled errors in a TaskGroup (1 sub-exception) (TimeoutError)`。根因:**npx/npm 在 Windows 不读系统代理**,产物 `mcp.py` 启动 npx 子进程时只透传 `os.environ`、未注入代理→npx 直连 npm 源被公司网络拦→`_npx` 缓存始终为空、拉不到包、撞 120s connect 超时;而 `uvx` 类能连(uv 的 urllib 探测天然走 `getproxies()` 代理)、Node 二进制也下得来(`urlretrieve` 同理),唯独 npx 不会。修复(产物运行时 `mcp.py`):`_mirror_env`(单镜像 + TCP 探测,弃用)→ **`_stdio_net_env(servers, config)`**:`_discover_proxy`(`mcp.proxy` 或 `urllib.getproxies()`)发现代理,对 `[override, 官方, 镜像]` × `[直连, 走代理]` 候选用 **HTTP 探测 `_url_ok`(proxy-aware opener)**取首个可达,注入 `npm_config_registry`/`UV_DEFAULT_INDEX`(+ 命中代理则 `HTTP(S)_PROXY`/`npm_config_proxy`),npm/uv 两族**并发探测**;`setdefault` 绝不覆盖用户已设值。新增运行期旋钮 `McpConfig.proxy/npm_registry/pip_index`(覆盖自动探测,仅值不收密钥)。**可读报错** `_connect_error`:stdio npx/uvx server 连接失败时,若无可达源→给"换源/设 mcp.proxy/预装/停用"建议、若已配置且超时→给"先 `mcp warm` 或调大 `connect_timeout_seconds`"。**预热** `prewarm_stdio_packages`(+ CLI `mcp warm`):用解析出的源/代理把包**只拉不起服务**(`npx -y --package <pkg> -- node -e 0` / `uvx --from <pkg> python -c ""`,fetch 完即退、不挂长寿命 server),把首次冷下载从 connect 超时里解耦;无可达源时直接报可读错、不 spawn(免空等)。启动脚本:`__launch_name__.bat` 顶部从 **WinINET 注册表**(`ProxyEnable`/`ProxyServer`,PowerShell)读系统代理填 `HTTP(S)_PROXY`(扁平 goto、无 `setlocal`/`call`),`.sh` 仅补注释(env 已覆盖)。向导一键启动:`_product_env()` 经 `_ensure_proxy_env` 注入系统代理(`uv sync` 也受益)。`node_bootstrap._reachable` 由 TCP 改 **HTTP HEAD**(与 `urlretrieve` 一样走代理)。**真机实测**:设 `HTTP_PROXY` 后 npx 经代理成功拉取 DC 全依赖树、`_npx` 缓存从空→填充、`npx --package … -- node -v` 18s 干净退出(exit 0)。
  - **预热作 CLI-only,不进向导 job(2026-06-10 复盘 §1 取舍)**:`mcp warm` 保留为**手动可选**命令(离线前预拉 / 显式暴露不可达源),`prewarm_stdio_packages` 用 fetch-only(`npx -y --package <pkg> -- node -e 0` / `uvx --from <pkg> python -c ""`,拉完即退、不挂 server;无可达源直接报错不 spawn)。**移除了向导的 `warm` job 步**——它会把"产物 Web 打开"阻塞到冷下载完成,而用户不会久留;改由"首连后台拉 + 产物页实时进度/增量工具"(下条)覆盖体验,job 步回到 `render→sync→node→serve`。
  - **首连实时进度 + 工具增量显示(2026-06-10,§3/§4)**:① 产物 MCP 管理页/Tools 页黄灯期不再"无反馈"——`McpManager` 给每 server 连接计时(`_connect_started`)并经 `stdio_client(errlog=临时文件)` 捕获子进程 stderr(npx/uvx 拉包输出),`status()` 增 `connecting_seconds`/`log_tail`;前端卡片在 amber 时显示"connecting… Ns · <最新 stderr 行>"。② `/mcp/discover` 改**非阻塞**(去掉 `wait_ready`,返回当前 `discovered` + 每 server `connecting/connected` + 顶层 `pending`),Tools 页轮询(`toolsPollTimer`,`pending` 为真且在 Tools 标签时续轮询),**哪个 server 先就绪就先显示其工具**,不被慢/失败的 server 卡住;就绪即停轮询、签名比对避免 poll 抹掉编辑中的勾选。临时日志文件 best-effort、失败降级(仅少进度、绝不挡连接)、server task 退出即清理。
  - 测试:产物 `test_mcp.py` 四场景(国外直连/国内镜像/企业代理/全不可达)选源选代理 + `_discover_proxy` 优先级 + 用户已设不覆盖 + `_warm_argv` 构造 + 预热不可达不 spawn + `_connect_error` 文案 + `status()` 含 `connecting_seconds`/`log_tail`;`test_web.py` `/mcp/discover` 非阻塞返回 `pending`/per-server 态 + 增量 UI 串(`toolsPollTimer`/`mcpDiscovered.pending`/`connecting_seconds`)+ 既有 discover/status 测试改为先 `wait_ready` 再断言;`test_wizard.py::test_ensure_proxy_env_fills_from_system_proxy` + 两 node-launch 测试(回到 `render→sync→node→serve`);`test_generator.py::test_mcp_product_has_package_fetch_resolver_and_warm`(含 resolver/`mcp warm`/旋钮/`.bat` 代理段,关 MCP 零痕迹);**golden web+mcp 端到端绿**(105 生成器侧 + golden 两项)。同批顺手修 5 个 Windows-only 测试可移植性(`read_text(encoding=utf-8)` / `S_IXUSR` 仅 POSIX 断言 / `node_bootstrap` 测试 pin `install_base`,均测试侧、非产物 bug)。

- **便携 Node 下载:多源 + 慢回退 + 代理(2026-06-10 续,用户实测:一键"准备 Node"卡在 nodejs.org 慢下载、无回退)**:`node_bootstrap` 的下载由"单源 `urlretrieve`(`官方 if _reachable else 镜像`,且 TCP 通≠下得快)"改为 **`urlopen` 流式多源**:`_sources(prefer_mirror)` 给有序候选 `[官方, 镜像]`/`[镜像, 官方]`(`HF_NODE_DIST` 可强制单源),`_download` 带**每读 30s 超时(卡住即弃)** + **慢启动早退(20s 还没到 25% 就换下一源)**,一个源失败/卡/慢就自动回退下一个;`ensure_portable_node(prefer_mirror=…)` 新增形参,**向导按 `not _pypi_reachable()` 传参**(墙内→镜像优先,免在慢官方源上白等)。代理:`urlopen` 默认 opener 天然走 `getproxies()`(env + Windows 系统代理),`_reachable`(8834aee 已改 urllib HEAD)同理;与 `_ensure_proxy_env`(uv sync / 产物 MCP 子进程的代理注入)分工。测试:`tests/test_node_bootstrap.py` 改 `urlopen` 化 + 多源回退/镜像优先/全失败/`HF_NODE_DIST` 覆盖;`test_wizard.py` 断 `prefer_mirror` 透传;真机验 npmmirror 流式下载 0→100% + 解压 `bin/node`。
