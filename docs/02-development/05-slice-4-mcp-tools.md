...
- **暂走重启**:server 列表的增删改——改 server 需重建 `McpManager` + 重连子进程/远程会话,是**有状态重连**;本片 server 列表经 `config.yaml` + 重启生效,**热重连排 v1+**(`00-overview §2` Slice 6+)。`/config` 对 server 可只读展示,不在本片做"改 server 即热重连"。
...
- **传输**:stdio + 远程 HTTP/SSE 都做(人 2026-06-03 定向);**联网 MCP registry、`/config` 改 server 热重连、`forge add` 增量接 server** 均为 v1+,不在本片(`00-overview §2` Slice 6+ / `01 §3` L3)。
