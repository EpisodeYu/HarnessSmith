...
| context 默认 | **strategy `summarize` + 触发条件 `triggers`(默认 `max_tokens: 192000`、不限轮数)**(人 2026-06-06 拍板,取代原"默认 `truncate`")。**条件可组合**(`combine: or/and`,内置 `max_tokens`/`max_turns`)、**策略/条件走薄注册表**(`@register_strategy`/`@register_condition`,运行期可自加、按名分发,**非抽象层**守红线);`summarize` 无 compaction 角色时**回落首个 profile(用 generation 模型做摘要),不回落 truncate**(实现说明 2026-06-08,见 test-report RT-1;摘要开销已计入 trace/预算,RT-2);`strategy: none` 或空 `triggers` = 不压。offload 仍 v1+ |
