...
| **Slice 8** 会话持久化 ★v1 必做 | (实现前细化) | 产物**会话落盘 + 续聊**:一次对话的 `messages` 落本地(`.harness/sessions/` 或 traces 旁,JSON/JSONL,**不落密钥**,沿用 trace 脱敏纪律)+ CLI `run --continue`/`--resume <id>`(或 `chat` 多轮 REPL)+ Web session 可续。**复用现有 `Trace` 基础设施、零新增依赖**;续聊历史预置进所选 paradigm 的初始 `messages`。**状态:📋 已登记(人 2026-06-07 定向),实现前细化** | 多轮会话存取(mock)绿;续聊历史正确预置;**会话文件不含密钥断言**;默认/关闭行为下仍薄、零新增依赖;`uv sync && pytest` 黄金路径绿 | ① 默认是否落盘 + 存放位置/格式;② 单轮 `--continue` vs `chat` 多轮 REPL 的 CLI 形态 |
| **Slice 8B** 跨会话记忆 ★v1 必做 | (实现前细化) | **跨会话续跑的持久长期记忆/笔记**:agent 可写入/读取一份**少量、自维护**的长期笔记(本地 JSON/markdown,`.harness/memory*`,**不落密钥**),跨会话注入回系统提示。**建在 Slice 8 会话落盘基础设施之上、复用其文件/脱敏纪律,零新增依赖**;spec 开关式薄实现,默认行为保持薄。**与 RAG 严格区分:记忆 = 自维护的少量长期笔记,RAG(v1+) = 外部语料检索**。**状态:📋 已登记(人 2026-06-07 上移 v1 + 定向紧接 Slice 8),实现前细化** | 记忆写入/读取/跨会话注入(mock)绿;**记忆文件不含密钥断言**;关闭零痕迹、零新增依赖;`uv sync && pytest` 黄金路径绿 | ① 记忆存储形态(单文件 JSON vs markdown 笔记)+ 写入触发(工具显式写 vs 自动摘要);② 默认开/关 + 注入预算上限(防把"薄"做胖) |
| **Slice 9** Checkpoints ★v1 必做(**与 Slice 10 配对:工具护栏对,相邻一起做**) | (实现前细化) | **危险操作前自动 git 快照 + 一键回滚**:启用写文件/shell 类工具时,在高风险工具执行**前**自动打 checkpoint(影子 git ref / `.harness/checkpoints/`)+ `<pkg> revert` + Web "撤销本次运行"。**复用 `before_tool` 挂点**(与 Slice 10 同),纯 own-code 调 `git` CLI、**零新增依赖**;spec/config 开关(或检测到写类工具自动启)。产物本就 `git init`,天然有底座。**状态:📋 已登记(人 2026-06-07 定向 + 与 Slice 10 配对),实现前细化** | 快照→改文件→`revert` 还原测试绿;无写类工具时零开销;关闭零痕迹;**文档讲清"回滚文件≠阻止破坏"**(强隔离仍交 Docker,威胁模型 B,`01 §4`) | ① 自动启 vs 显式开关;② 快照存储形态(影子 git ref vs 目录) |
| **Slice 10** HITL 交互确认 ★v1 必做(**与 Slice 9 配对:工具护栏对,共用 `before_tool` + Web SSE 审批管道**) | (实现前细化) | **工具调用交互式确认**:内置可选 `ConfirmHooks(Hooks)` — 交互式 TTY/Web 弹 **allow / reject / always-allow**(记本会话或写回 `config.yaml`);**非交互 / Web 公开面默认拒绝**;`config.yaml tools.confirm` 选哪些工具需确认;default-off 零痕迹。**复用现有 `before_tool` 挂点**(`Hooks` 已就位,文档已注明 HITL 确认 live here)。让危险工具敢"预置不放行"。**与 Slice 9 共用 `before_tool` 挂点 + Web 审批往返管道,相邻一起做避免重复搭建**。**状态:📋 已登记(人 2026-06-07 定向,取代原 Slice 8+ 内嵌登记 + 与 Slice 9 配对),实现前细化** | allow/reject/always-allow 三态(mock TTY/Web)绿;**非交互默认拒绝断言**;关闭零痕迹;**文档讲清"护栏≠安全边界"**(威胁模型 A;锁能力仍靠生成期不编译,`01 §4`) | ① **是否据此把 shell 默认开 = 改 `01 §6` 全局口径,需人签字**(沿用既有登记);② always-allow 记到会话 vs `config.yaml` 的默认 |
| **Slice 11** MCP 健康/管理(三件套+记忆后首要方向,人 2026-06-07 定向) | (实现前细化) | **MCP server 健康自检 + 管理面**:① `<pkg> mcp status` / 扩 `doctor` + `/config` 健康视图——probe 每个配置 server 连通性 + 工具计数 + 不可达标红 + 缺 Node 提示,run-start 补记 skipped;② 面板内**增删/编辑 MCP server**(= `/config` 可改 `mcp`,**触安全面**,见决策点)+ 热重连(重建 `McpManager` + 重连子进程/远程会话)。**复用现有 `McpManager.errors`/`discovered`**(MCP 工具自动发现已于 2026-06-07 落地 Slice 3,见 `04-slice-3-product-web.md §4`),对齐 Cursor MCP 面板。**状态:📋 已登记(人 2026-06-07 升格为 Slice 11),实现前细化** | `mcp status`/`doctor` 连通性+工具计数+不可达标红(mock/真实 stdio)绿;面板增删 server → 热重连生效(mock)绿;关 MCP 时零痕迹;`uv sync && pytest` 黄金路径绿 | ① 面板改 `mcp`(增删/编辑 server)= `/config` 触安全面,**须与公开面隔离前提**(沿用 Slice 13+ "发布拓扑隔离"登记);② 热重连失败时的回退/报错形态 |
| **Slice 12** 原生 Anthropic 双规范 + 推理流式 UX(人 2026-06-07 升格 + §6.4 方案待签) | [`../05-llm-dual-spec-anthropic.md`](../05-llm-dual-spec-anthropic.md) | **`llm.py` 第二个 client 走原生 Anthropic Messages**(顶层 `system` / content blocks / `tool_use`+`tool_result` / `effort`+adaptive thinking / Opus 4.7/4.8 禁 `temperature`·`top_p`·`top_k` / prompt caching `cache_control` / structured outputs),把 loop 的消息·工具格式映射进/出;**spec 开关式可选模块,默认仍 OpenAI 兼容端点、`pyproject` 不进 `anthropic`、关闭零痕迹**。同时把 **reasoning/thinking 阶段做成显式流式提示**(`event: thinking` / 思考流推送,治 2026-06-03 真实 LLM 验收发现的"无反应等待")。**状态:📋 §6.4 方案已写出待人签(见子文档),签字后实现** | 双规范 client 消息/工具格式映射往返(mock)绿;默认 provider-agnostic 路径不变、关闭零痕迹、`pyproject` 不含 `anthropic`(薄验证);推理流式提示(mock)绿;`uv sync && pytest` 黄金路径绿 | ① **改 LLM API 面 = `CLAUDE.md §6.4`,实现前人签 `05-llm-dual-spec-anthropic.md` 的具体方案**;② 思考流是 `reasoning_content` 透传 vs 独立 `event: thinking` |
| **Slice 13+ (v1+)** | (暂不立子文档;差异化两项见 `04-forge-add` / `03 §6 D-2`) | **`forge add` / 增量再生成 + 模板升级**(D-1,**唯一结构性护城河**,先做 Phase 1 安全子集;详设见 [`../04-forge-add-incremental-regeneration.md`](../04-forge-add-incremental-regeneration.md))、**内置极薄 eval / golden-task harness**(D-2,spec 开关,补"验证"支柱、可拥有 eval 的差异化空位;详见 `03 §6 D-2`)、**更多 preset + spec 分享**(D-3,`deep-research`/`customer-support`/`data-analyst` 等,低成本低风险、可随时插)、**supervisor multi-agent**(agent 即 tool,固定拓扑,opt-in)、**任务清单跟踪**(TodoWrite 式,`03 §4 T2-F`)、**周期预算持久化层**(天/周/月配额 + 跨 run 累计 = 持久用量账本,spec 勾选的可选模块;per-run 条件/时间窗/注册表已于 2026-06-07 落地)、RAG ingest + sqlite-vec、**联网 MCP registry**(对齐"不自建在线 registry"红线,靠 marketplace 文档 + remote MCP;MCP 健康/管理本体已升格 Slice 11)、**发布拓扑:`/config` 与公开面隔离**(管理面鉴权 / 绑 localhost / 生成期开关,使"管理员托管 + 接口发布"时 `/config` 不可被终端用户访问;人 2026-06-05 登记,见 `04-slice-3-product-web.md §4`;**Slice 11 面板改 `mcp` 落地前需此前提**)、`/config` 热重载进阶、keyring、**配置级 shell hook**(Claude Code/Cursor 风格:`config.yaml` 声明事件→shell 命令、`PreToolUse` 可 veto / `PostToolUse` 跑 formatter;**代码级 `Hooks`(Slice 1)已覆盖同等能力**,且与 Slice 10 HITL 确认重叠、引入"配置可间接 exec 任意命令"新安全面,故缓做;人 2026-06-05 登记,见 `075-slice-6b-global-rules.md §5`)、context offload、sandbox-aware shell 工具 wrapper(`03 §4 T2-H`,谨慎、先文档)、自定义 slash command / 插件 marketplace(`03 §5`,低优先) | 各项做到即验(见 `01 §7 Non-blocker`) | **不在 MVP**;进入前需人决定排期 |
...
| **LLM API 面** | **Chat Completions + `tools`,不用 Responses**(兼容第三方 / 本地 OpenAI 兼容端点)。接 Claude 走 Anthropic OpenAI 兼容端点 / LiteLLM。**原生 OpenAI+Anthropic 双规范可切 = 可选模块,2026-06-07 升格为 Slice 12**(§6.4 具体方案已写出待人签,见 [`../05-llm-dual-spec-anthropic.md`](../05-llm-dual-spec-anthropic.md);默认仍 provider-agnostic Chat Completions,双规范是可选第二条而非替换;改本面需人签 `CLAUDE.md §6.4`) |
...
| 安全(轻量,本地自用) | 密钥不入 git(`config.yaml`/`harness.spec.yaml` 只存 env 引用名,真值放 `.env`);高风险工具(shell/写文件)默认关,仅 allowlist 显式开;沙箱 / keyring / 全链路 redaction 推迟。**产物提供 write-only `.env` 助手**(Slice 7:CLI `set-key` + Web `POST /env`/LLM tab)把 key 真值**只写本地 gitignored `.env`、不回显、不进 git/spec/trace**——`.env` 本就是放真值处,合规;真·密钥库(keyring/OS 凭证)仍 v1+。生成器 wizard 始终不收 key |
...
| context 默认 | **strategy `summarize` + 触发条件 `triggers`(默认 `max_tokens: 192000`、不限轮数)**(人 2026-06-06 拍板,取代原"默认 `truncate`")。**条件可组合**(`combine: or/and`,内置 `max_tokens`/`max_turns`)、**策略/条件走薄注册表**(`@register_strategy`/`@register_condition`,运行期可自加、按名分发,**非抽象层**守红线);`summarize` 无 compaction 角色时回落 truncate;`strategy: none` 或空 `triggers` = 不压。offload 仍 v1+ |
...
不做:生产级权限系统、云托管、**通用多 agent 编排框架 / 工作流编排 DSL / 动态图引擎 / 运行期范式抽象层**、在线 MCP registry、沙箱、**HarnessForge 侧中心化配置/托管**;以及 L3/v1+ 项(RAG / 联网 MCP registry / `/config` 热重载 / keyring / 完整 HITL Web / context offload / **一种 supervisor multi-agent** / 周期预算 / **跨会话全局记忆**(非红线,人 2026-06-07 定向上移 v1 必做、紧接会话持久化记为 **Slice 8B**,见 §2)/ **会话持久化·Checkpoints·HITL 确认**(已升格为 v1 必做 Slice 8/9/10)/ **MCP 健康/管理**(升格 Slice 11)/ **原生 Anthropic 双规范 + 推理流式**(升格 Slice 12,§6.4 待签))在 MVP 内不做。(注:**多范式(单 loop Agent/Plan/Ask)已进 Slice 5、工具基线 + 标准 SKILL 进 Slice 6**——它们不是被禁项;被禁的是"通用多 agent 编排框架 / 范式抽象层"。MCP 远程 HTTP/SSE 传输已于 2026-06-03 提前进 Slice 4/L2。)
