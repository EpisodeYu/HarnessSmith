...
| **Slice 7** wizard + 产物分页配置 | [`08-slice-7-wizard.md`](./08-slice-7-wizard.md) | ① **生成器 wizard**(`harnessforge/wizard/`,FastAPI + 无构建单页,`[wizard]` extra):**首项选语言(中/英)+ 按功能分页 + 输入显示名自动派生 slug**,采集 `HarnessSpec` 全字段(含 `paradigms`/`mcp.enabled`+catalog/`skills.enabled`)→ `POST /spec` 校验产合法 spec + 可选一键 `generate()`;只采集 env 名、不进产物。② **产物分页配置页**:把 Slice 3 的 `/config` 重组成**按功能 tab**(LLM/Context/Budget/Tools/Prompts/Paradigms/Observability)+ **中英切换**,只改运行期行为性配置(产物**不做 wizard/首启**)。新增 `spec.display_name`(纯标签)+ `spec.language`(产物 web 默认 UI 语言,**wizard 语言选择贯穿至此**;Agent 回答语言另由 wizard 可选软指令注入系统提示);`spec.context` 起种子化 `config.yaml`。**状态:✅ 已完成(2026-06-06;快测 97 + golden wizard 端到端;ReadLints clean)** | wizard 产合法 spec 能 `generate` 出可跑产物(golden);不泄密/不进产物(核心 `dependencies` 无 fastapi/uvicorn);catalog 预填经 wizard 落 `config.yaml`;display_name 渲染进标题/README;产物配置分页 + 中英;字段对外可读(人审②) | ① 全覆盖+按功能分页 ✅(人 2026-06-05);② **2026-06-06 细化**:语言优先+双语、显示名派生 slug(加 `display_name`,§6.1 已签)、产物只分页配置不做 wizard、LLM 维持 provider-agnostic + **原生双规范登记 v1+** ✅;③ 对外可读(实现后真实验收) |
| **Slice 8+ (v1+)** | (暂不立子文档) | **原生 OpenAI+Anthropic 双规范 LLM client**(spec 开关式可选模块,人 2026-06-06 登记,Slice 7 讨论分出)——`llm.py` 第二个 client 走 Anthropic Messages(顶层 `system` / content blocks / `tool_use`+`tool_result` / `effort`+adaptive thinking / Opus 4.7/4.8 禁 `temperature`·`top_p`·`top_k` / prompt caching `cache_control` / structured outputs),把 loop 的消息·工具格式映射进/出;改 LLM API 面 = `CLAUDE.md §6.4`,实现前需人再签具体方案。**当下接 Claude 走 Anthropic OpenAI 兼容端点 / LiteLLM,本片不做**、**supervisor multi-agent**(agent 即 tool,固定拓扑,opt-in)、**周期预算**(天/周/月,持久化可选模块)、RAG ingest + sqlite-vec、**联网 MCP registry / `/config` 改 MCP server 热重连 / `forge add` 增量接 server**(MCP 远程 HTTP/SSE 传输本身已提前进 Slice 4)、**发布拓扑:`/config` 与公开面隔离**(管理面鉴权 / 绑 localhost / 生成期开关,使"管理员托管 + 接口发布"时 `/config` 不可被终端用户访问;人 2026-06-05 登记,见 `04-slice-3-product-web.md §4`)、`/config` 热重载进阶、keyring、完整 HITL Web、**工具调用 HITL 确认**(allow/reject/always-allow;内置可选 `before_tool` 确认 hook,**今天已可经 `before_tool` hook `raise`-veto 自实现**、v1+ 做成内置 + 干净 allow/deny 返回;**非交互/Web 必须默认拒绝**;**是否据此把 shell 默认开 = 改 §6 全局口径,待人签字**,人 2026-06-05 登记)、**配置级 shell hook**(Claude Code/Cursor 风格:`config.yaml` 声明事件→shell 命令、`PreToolUse` 可 veto / `PostToolUse` 跑 formatter;**代码级 `Hooks`(Slice 1)已覆盖同等能力**,且与上条 HITL 确认重叠、引入"配置可间接 exec 任意命令"新安全面,故缓做;人 2026-06-05 登记,见 `075-slice-6b-global-rules.md §5`)、**MCP 状态自检/健康标注**(probe 每个配置 server 连通 + 工具计数 + 不可达标红 + 缺 Node 提示;CLI `<pkg> mcp status`/扩 `doctor` + `/config` 健康视图,对齐 Cursor MCP 面板;**复用现有 `McpManager.errors`/`discovered`**、run-start 补记 skipped;人 2026-06-05 登记)、context offload、**推理模型流式 UX**(reasoning 阶段显式提示/思考流,避免无反应等待;2026-06-03 真实 LLM 验收发现,详见 `04-slice-3-product-web.md §4`) | 各项做到即验(见 `01 §7 Non-blocker`) | **不在 MVP**;进入前需人决定排期 |
...
| **LLM API 面** | **Chat Completions + `tools`,不用 Responses**(兼容第三方 / 本地 OpenAI 兼容端点)。接 Claude 走 Anthropic OpenAI 兼容端点 / LiteLLM。**原生 OpenAI+Anthropic 双规范可切 = v1+ 可选模块**(人 2026-06-06 登记,见 §2 Slice 8+;改本面需人再签 `§6.4`) |
