...
| **Slice 9** Checkpoints ★v1 必做 | (实现前细化) | **危险操作前自动 git 快照 + 一键回滚**:启用写文件/shell 类工具时,在高风险工具执行**前**自动打 checkpoint(影子 git ref / `.harness/checkpoints/`)+ `<pkg> revert` + Web "撤销本次运行"。**复用 `before_tool` 挂点**(与 Slice 10 同),纯 own-code 调 `git` CLI、**零新增依赖**;spec/config 开关(或检测到写类工具自动启)。产物本就 `git init`,天然有底座。**状态:📋 已登记(人 2026-06-07 定向),实现前细化** | 快照→改文件→`revert` 还原测试绿;无写类工具时零开销;关闭零痕迹;**文档讲清"回滚文件≠阻止破坏"**(强隔离仍交 Docker,威胁模型 B,`01 §4`) | ① 自动启 vs 显式开关;② 快照存储形态(影子 git ref vs 目录) |
| **Slice 10** HITL 交互确认 ★v1 必做 | (实现前细化) | **工具调用交互式确认**:内置可选 `ConfirmHooks(Hooks)` — 交互式 TTY/Web 弹 **allow / reject / always-allow**(记本会话或写回 `config.yaml`);**非交互 / Web 公开面默认拒绝**;`config.yaml tools.confirm` 选哪些工具需确认;default-off 零痕迹。**复用现有 `before_tool` 挂点**(`Hooks` 已就位,文档已注明 HITL 确认 live here)。让危险工具敢"预置不放行"。**状态:📋 已登记(人 2026-06-07 定向,取代原 Slice 8+ 内嵌登记),实现前细化** | allow/reject/always-allow 三态(mock TTY/Web)绿;**非交互默认拒绝断言**;关闭零痕迹;**文档讲清"护栏≠安全边界"**(威胁模型 A;锁能力仍靠生成期不编译,`01 §4`) | ① **是否据此把 shell 默认开 = 改 `01 §6` 全局口径,需人签字**(沿用既有登记);② always-allow 记到会话 vs `config.yaml` 的默认 |
| **Slice 11+ (v1+)** | (暂不立子文档) | **跨会话全局记忆**(v1 必做,人 2026-06-07 定向;跨会话续跑的持久记忆/笔记,薄实现 + spec 开关,**与 RAG 区分:记忆=自维护的少量长期笔记,RAG=外部语料检索**;排期与拆片实现前细化)、**原生 OpenAI+Anthropic 双规范 LLM client**(spec 开关式可选模块,人 2026-06-06 登记,Slice 7 讨论分出)——`llm.py` 第二个 client 走 Anthropic Messages(顶层 `system` / content blocks / `tool_use`+`tool_result` / `effort`+adaptive thinking / Opus 4.7/4.8 禁 `temperature`·`top_p`·`top_k` / prompt caching `cache_control` / structured outputs),把 loop 的消息·工具格式映射进/出;改 LLM API 面 = `CLAUDE.md §6.4`,实现前需人再签具体方案。**当下接 Claude 走 Anthropic OpenAI 兼容端点 / LiteLLM,本片不做**、**supervisor multi-agent**(agent 即 tool,固定拓扑,opt-in)、**周期预算持久化层**(天/周/月配额 + 跨 run 累计 = 持久用量账本,spec 勾选的可选模块;per-run 条件/时间窗/注册表已于 2026-06-07 落地)、RAG ingest + sqlite-vec、**MCP 健康管理**(联网 MCP registry / 面板内增删·编辑 MCP server = `/config` 可改 `mcp`,触安全面 / server 连通性·健康状态面板 #19 T2-G / 热重连 / `forge add` 增量接 server;**MCP 工具自动发现已于 2026-06-07 提前落地 Slice 3**——`GET /mcp/discover` + Tools 面板扫描勾选,见 `04-slice-3-product-web.md §4`;MCP 远程 HTTP/SSE 传输已提前进 Slice 4)、**发布拓扑:`/config` 与公开面隔离**(管理面鉴权 / 绑 localhost / 生成期开关,使"管理员托管 + 接口发布"时 `/config` 不可被终端用户访问;人 2026-06-05 登记,见 `04-slice-3-product-web.md §4`)、`/config` 热重载进阶、keyring、完整 HITL Web(**工具调用 HITL 交互确认已升格为 Slice 10**,见上)、**配置级 shell hook**(Claude Code/Cursor 风格:`config.yaml` 声明事件→shell 命令、`PreToolUse` 可 veto / `PostToolUse` 跑 formatter;**代码级 `Hooks`(Slice 1)已覆盖同等能力**,且与上条 HITL 确认重叠、引入"配置可间接 exec 任意命令"新安全面,故缓做;人 2026-06-05 登记,见 `075-slice-6b-global-rules.md §5`)、**MCP 状态自检/健康标注**(probe 每个配置 server 连通 + 工具计数 + 不可达标红 + 缺 Node 提示;CLI `<pkg> mcp status`/扩 `doctor` + `/config` 健康视图,对齐 Cursor MCP 面板;**复用现有 `McpManager.errors`/`discovered`**、run-start 补记 skipped;人 2026-06-05 登记)、context offload、**推理模型流式 UX**(reasoning 阶段显式提示/思考流,避免无反应等待;2026-06-03 真实 LLM 验收发现,详见 `04-slice-3-product-web.md §4`) | 各项做到即验(见 `01 §7 Non-blocker`) | **不在 MVP**;进入前需人决定排期 |

> **循环范式 / multi-agent(候选,人已定向)**:默认 loop 是单一原生 function-calling(ReAct/TAO),薄+无框架的核心卖点。扩展走 **wizard 生成期多选 + 产物侧薄范式注册表**(人 2026-06-05 定向,Slice 5):① **单 loop 范式集 Agent(默认)/ Plan / Ask**(对齐 Cursor 三件套;`agent` = ReAct 式 tool-calling 循环,**初版 react/plan/ask/reflection 当日修订为此**,见子文档 §4②′)——生成期**多选**进产物**共存**,**产物运行期每轮选一种**(类 Cursor agent/ask/plan;CLI `--mode` + Web 下拉;运行期清单 + 默认进 `config.yaml`,首项种默认);**Plan/Ask 只读**(只 offer 只读/低风险工具、禁 write/shell;Plan 对齐 Cursor 只产只读计划不动手,Build 切换推迟)。**反思**不作独立范式:agent 循环已"在线自纠"(工具报错喂回→重试),事后 Reflexion(验证器门控重试)作 `AGENTS.md` 用户扩展范例。**② 范式可扩展(核心卖点)**——产物 `harness/paradigms/` 持**与 tools 同款的薄注册表 + `@register_paradigm` 装饰器**,**运行期用户可自加范式**(写函数+注册+配 `enabled`);**内置范式各自自包含、互不 import**(改 agent 不影响 ask);**注册表始终存在**(连只选 agent 也有)→ 默认产物不再与 Slice 1–4 逐字一致(门禁改"行为一致+无新增依赖");**扩展性/解耦优先于薄**。运行期"每轮选范式" + "范式注册表" **实现为已注册模式集的写死按名分发(own-code),不是被禁的"运行期范式抽象层"/动态图/DSL/编排引擎**。③ **一种 supervisor multi-agent 模式**(agent 即 tool,固定拓扑,生成为自有代码,opt-in)——**排 v1+(Slice 11+)**;用户自写 multi-agent 范式属其 own-code。**红线**:不做通用多 agent 编排框架 / 工作流 DSL / 动态图引擎 / 运行期范式抽象层(`01 §6`)。详见 `06-slice-5-paradigms.md`。
>
...
| **LLM API 面** | **Chat Completions + `tools`,不用 Responses**(兼容第三方 / 本地 OpenAI 兼容端点)。接 Claude 走 Anthropic OpenAI 兼容端点 / LiteLLM。**原生 OpenAI+Anthropic 双规范可切 = v1+ 可选模块**(人 2026-06-06 登记,见 §2 Slice 11+;改本面需人再签 `§6.4`) |
...
不做:生产级权限系统、云托管、**通用多 agent 编排框架 / 工作流编排 DSL / 动态图引擎 / 运行期范式抽象层**、在线 MCP registry、沙箱、**HarnessForge 侧中心化配置/托管**;以及 L3/v1+ 项(RAG / 联网 MCP registry / `/config` 热重载 / keyring / 完整 HITL Web / context offload / **一种 supervisor multi-agent** / 周期预算 / **跨会话全局记忆**(非红线,人 2026-06-07 定向上移 v1,见 §2 Slice 11+)/ **会话持久化·Checkpoints·HITL 确认**(已升格为 v1 必做 Slice 8/9/10))在 MVP 内不做。(注:**多范式(单 loop Agent/Plan/Ask)已进 Slice 5、工具基线 + 标准 SKILL 进 Slice 6**——它们不是被禁项;被禁的是"通用多 agent 编排框架 / 范式抽象层"。MCP 远程 HTTP/SSE 传输已于 2026-06-03 提前进 Slice 4/L2。)
