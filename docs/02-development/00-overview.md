...
| Multi-agent 子代理（`subagents.enabled`） | agent-as-tool / orchestrator-worker：主 agent 经 `subagents` 工具把独立子任务并行委派给作用域受限的 worker（独立上下文 + 各自工具/模型），固定拓扑、深度 1；非新范式、不改核心循环；向导默认开（详见 [`16-slice-14-subagents.md`](./16-slice-14-subagents.md)） |

### v1+ backlog（不在当前范围）

`forge add` 增量再生成 + 模板升级（结构性护城河，详见 [`14-forge-add-incremental-regeneration.md`](./14-forge-add-incremental-regeneration.md)）、内置极薄 eval / golden-task harness、更多 preset + spec 分享、任务清单跟踪、周期预算（天/周/月配额，复用现有 cost 账本）、LLM 支持线剩余项（structured outputs 默认路径、滚动摘要/microcompaction/缓存友好压缩，详见 [`15-llm-robustness-and-context.md`](./15-llm-robustness-and-context.md)）、RAG ingest + sqlite-vec、联网 MCP registry、发布拓扑「`/config` 与公开面隔离」、`/config` 热重载进阶、keyring、配置级 shell hook、context offload、sandbox-aware shell 工具 wrapper、自定义 slash command / 插件 marketplace、capability bundle（一个模块声明一组工具/hooks/prompt 加项的接线糖，现有 `extensions:` + `@register_*` 已覆盖 80%，issue #6 暂缓）、LLM provider registry（`@register_provider` 让新增 provider 不改 `llm.py`，openai-compatible + 原生 anthropic 已覆盖绝大多数，issue #6 暂缓）。
...
- **可扩展（与无框架同级的核心卖点）**：薄抽象、清晰模块边界（loop / llm / tools / context 各单一职责，可独立替换）；工具、范式、上下文策略/条件、工具结果截断器、记忆后端都走**同款薄注册表 + 装饰器**（`@tool`/`@register_paradigm`/`@register_strategy`/`@register_condition`/`@register_truncator`/`@register_memory`）——新增 = 写函数 + 注册 + 配 `config.yaml`，不改核心循环；生命周期 hooks（`before_step`/`after_step`/`before_tool`/`after_tool`/`on_error`）挂护栏/日志/脱敏不动核心，且**可 list 组合 + 参数化**（`config.hooks` 接受旧字符串或 `[{class,enabled,config}]`），其中 `before_tool` 返回字符串=拒绝该工具调用（先于 HITL、只收窄不扩张）、`after_tool` 返回字符串=改写/脱敏结果（模型与 trace 同步生效）——即薄工具策略层，不另建编排框架；关键扩展点（含已挂 hooks、已 import extensions）经 `info` / `GET /registries` 可发现，Web 独立 Hooks 页（含执行日志）编辑 hooks、System 页编辑 extensions（本地可信面）。
...
- `harnessmith/spec.py` — `HarnessSpec`（Pydantic v2 + YAML，带 `version`，`extra="forbid"`）。字段：`version` / `project_slug` / `display_name`（人类可读显示名，1–120 字符、单行且无控制字符；按 Python/HTML/Markdown/脚本上下文分别编码，空则回落 slug；wizard 由它派生 slug）/ `language`（产物 web 默认 UI 语言 `en`/`zh`）/ `llms`（每 profile 含 `provider: openai|anthropic`，默认 `openai`；`api_key_env`/`base_url_env` 只接受 env 名）/ `roles` / `prompts`（`system` + `rules_files` 种子）/ `tools` / `paradigms`（多选 `Literal["agent","plan","ask"]`，默认 `["agent"]`）/ `interfaces`（`cli`/`web`/`tui`）/ `mcp.enabled` / `skills.enabled` / `memory.enabled` / `subagents.enabled`（multi-agent 子代理，agent-as-tool，opt-in，默认关）/ `observability` / `context`（种子）/ `rag` / `secrets` 预留。
...
- `src/<pkg>/harness/` — `config` / `loop` / `llm` / `llm_anthropic` / `tools` / `trace` / `usage` / `debuglog` / `prompts` / `hooks` / `session` / `interaction`（+ `context`、`paradigms/`、`mcp.py` opt-in、`skills.py` opt-in、`memory.py` opt-in、`subagents.py` opt-in、`rag.py` v1+）。
...
| 安全（轻量，本地自用） | 密钥不入 git：`config.yaml`/`harness.spec.yaml` 只存 env 引用名，真值放 `.env`；生成期 spec/catalog 与产物运行期 LLM/MCP config 对 `api_key_env`/`base_url_env`/`env`/`auth_env` 统一只接受 `^[A-Za-z_][A-Za-z0-9_]*$`，并拒绝常见 secret-looking token；错误先脱敏，不回显被误贴的值。产物提供 write-only `.env` 助手（CLI `set-key` + Web `POST /env`/LLM tab），真值不进 git/spec/trace。高风险工具（shell/写文件）默认关，仅 allowlist 显式开；沙箱 / keyring / 全链路 redaction 推迟。**Web 本地边界**：默认只接受 loopback Host + 同源 `Origin`/`Referer`/`Sec-Fetch-Site`；根页面签发每进程随机、Strict HttpOnly session cookie，所有变更另需随机 CSRF header；chat 由 `POST /chat` 创建不可猜测 run，`GET /chat/{run_id}/events` 只订阅；非 loopback bind 必须显式 `--unsafe-allow-remote`，共享部署仍需独立认证。**例外**：wizard 产物默认启用 desktop-commander 并默认 `confirm: high`，仅限 wizard；`coding-assistant` preset / CLI 仍维持高风险默认关。**MCP 管理面增删/编辑 server = 新安全面**（可 spawn 任意本地命令），`/config`/`/mcp`/`/fs` 仍按本地可信定位。 |
| MCP | **stdio 本地 + 远程 HTTP/SSE 传输**；生成期只决定 on/off（`spec.mcp.enabled` + `mcp` 依赖），server/tool/传输全运行期 `config.yaml`（用户可自带 server，无白名单；安全闸 = tool allowlist + 风险标记 + 密钥按 env 名）。catalog = MCP 预设静态精选 + wizard/CLI 预填数据源（非编译进产物、非安全闸）；**因产物无内置实用工具，MCP 预设兼做「基线能力来源」**（`fetch` / `web-search`（免 key 多引擎、自动 failover；`ddg-search` 留 catalog 作 uvx 备选）/ `git` / Desktop Commander，离线靠生成期预热 + Docker 烤镜像；Node 系 server 装一次后 `node` 直跑、不走 npx 临时缓存），另含带 key 候选 `bocha`（墙内合规搜索）/ `jina-reader`（复杂网页阅读,remote）作 opt-in 补充（向导末位默认不勾）；海量扩展走 marketplace 文档 + Composio 式 remote MCP。联网 MCP registry / `forge add` 仍 v1+ |
| 范式 / multi-agent | 默认 `agent`（ReAct 式 tool-calling 循环）；生成期多选 Agent/Plan/Ask（对齐 Cursor 三件套）渲染共存薄循环、运行期每轮选一种（CLI `--mode` + Web 下拉；`enabled`+`default` 进 `config.yaml`，首项种默认）；Plan/Ask 只读（只 offer 非高风险工具 + 执行期拒绝集合外调用）。**范式可扩展 = 与 tools 同款薄注册表 + `@register_paradigm`**，运行期用户可自加；内置范式各自自包含、互不 import；注册表始终存在。反思不作独立范式（agent 已在线自纠；Reflexion 作 `AGENTS.md` 用户扩展范例，需真实成功信号）。**multi-agent 已落地为 `subagents` opt-in 模块（agent-as-tool / orchestrator-worker，非新范式）**：supervisor = 默认 `agent` 范式自主调 `subagents` 工具，把独立子任务**并行**委派给作用域受限 worker（独立上下文 + 各自工具/profile），**固定拓扑、深度 1**、生成为自有代码、关闭零痕迹、零新增依赖（stdlib 线程池），详见 [`16-slice-14-subagents.md`](./16-slice-14-subagents.md)；**仍禁**通用多 agent 编排框架 / 工作流 DSL / 动态图引擎 / 运行期范式抽象层 |
...
| 对话级工作目录（issue #3） | **每 session 选一个工作目录，仅作上下文注入系统提示供 LLM 参考、非围栏**。`build_system_prompt(config, working_dir=None)` 拼一条独立注记（"Working directory (for reference): …; a hint, NOT a restriction"），不设=逐字节 no-op、与 `spec.mcp` 门控解耦。**不动 `HarnessSpec` schema**（纯运行期/session 级，默认进程 cwd）；**始终注入但默认 no-op**。**不 `os.chdir`、不做路径围栏/沙箱**（Web 单进程多 session 靠每轮按 session 注入避免串台）。透传链：CLI `--cwd` / `chat` 的 `/cwd`、Web `POST /chat {cwd}` → `loop.run(working_dir=)` → 三范式 → `build_system_prompt`；随 session 落盘（`session.cwd_of`，老 session 无字段安全回落）。Web 目录浏览 `GET /fs/list`（仅列子目录名、本地可信）。红线：真隔离仍靠 Docker / 生成期不编译。 |
| 技能 | 支持 **Agent Skills 开放标准**（`SKILL.md` + 渐进披露 L1 发现注入 / L2 文件工具或 `read_skill` 读正文 / L3 脚本经工具跑），`spec.skills.enabled` 门控、不引框架；技能脚本 = 高风险默认关。**L1 注入与 `read_skill` 正文均带技能目录绝对路径**，使正文里的相对引用（`scripts/`/`references/`/`assets/`）无需模型现场发现目录即可解析；L3 跑脚本/读资源依赖产物装有文件/shell 工具（基线由 MCP 提供，MCP 关则够不着）。**扩展可发现性**：产物 `GET /registries` + CLI `info` 内省注册表（`PARADIGMS`/`STRATEGIES`/`CONDITIONS`/`memory backends` + 已 import `extensions` 模块 + 已挂 `hooks`），Web 配置页据此渲染下拉/勾选列表、对未注册名字标 ⚠；Web 独立 **Hooks 页**（MCP 页之后）挂载/编辑 hooks 并显示**执行日志**（`GET /hooks/log`：每个已挂 hook 何时被调用，仅名称/方法/时间），**System 页**编辑 `extensions`；均本地可信面，编辑 `extensions` = 触发本机 import，勿公网暴露 `/config` |
| context 默认 | strategy `summarize` + 触发条件 `triggers`；**默认触发 usage 驱动的 `window_pct: 0.85`**（真实上一步 `prompt_tokens` ÷ 该 profile 的 `context_window`；`context_window` 默认不填 → 该条件不触发，靠工具结果截断 + 溢出自救兜底，不误伤大窗口模型），另有 `max_tokens`（绝对值，真实 usage 优先、字符估算兜底首调用）、`max_turns`。条件可组合（`combine: or/and`），策略/条件走薄注册表（`@register_strategy`/`@register_condition`，运行期可自加，非抽象层；自定义条件旧 2 参签名向后兼容）。`summarize` 无 compaction 角色时回落首个 profile。`max_tool_result_chars`（默认 200k 字符 ≈ 50k token，0=关）为入历史前第一道闸；`tool_result_strategy`（默认 `middle` 保留头尾、丢中间，另有 `head`/`tail`，走薄注册表 `@register_truncator`，对齐 Claude Code 的中间截断）决定如何截断。压缩保留地板取 `keep_last_turns`（user 轮，默认 6）与 `keep_last_steps`（tool-round，默认 8）中更激进者——后者让单个长 agentic 轮也能按轮折叠（issue #5 L1，见 [`15-llm-robustness-and-context.md`](./15-llm-robustness-and-context.md) §2.6）。offload 仍 v1+ |
...
| 预算 | **按 LLM 持久 cost 账本 + 限额**：产物 `harness/usage.py` 的 `UsageLedger` 按 profile name 把 token 用量持久累计到 `.harness/usage.json`（跨 run/重启；统计口径 = 主 generation + compaction，经 `Trace.add_usage` 记账；title/memory 不计），cost 由各 profile 当前单价派生（`input/output/cached_input_cost_per_million`）。每 profile `cost_limit`（同币种，0=不限）；累计 cost ≥ cost_limit → 调用前阻止该 LLM 并优雅停本轮（`stop_reason="llm_budget"`）+ 提醒（block_stop，不自动 fallback）。单价 + 限额 = 运行期活旋钮（Web Budget 页唯一入口 / CLI `usage`），spec 无 budget 块；清空手动。天/周/月周期配额仍 v1+（复用同账本）。另有与成本无关的循环步数硬上限 `max_steps`（运行期 `Config` 顶层字段，默认 50，0=不限，Web 在 **Context 页**显式可改）：到顶以 `stop_reason="max_steps"` 干净停，兜底「默认单价=0 → `cost_limit` 形同虚设」时的跑飞循环（issue #5 L4） |
...
graph LR
    ...
    S13 --> S14["Slice 14 multi-agent<br/>subagents (agent-as-tool, opt-in)"]
    S14 --> V["v1+<br/>forge add / eval / presets / RAG / …"]
```
...
| **Slice 6** 工具基线 + SKILL | [`07-slice-6-tools-and-skills.md`](./07-slice-6-tools-and-skills.md) | MCP 预设做基线（fetch/web-search/git/Desktop Commander）+ catalog + 离线/Docker；标准 Agent Skills（`SKILL.md` 渐进披露） |
...
| **Slice 14** multi-agent | [`16-slice-14-subagents.md`](./16-slice-14-subagents.md) | subagents（agent-as-tool / orchestrator-worker）：主 agent 经 `subagents` 工具并行委派给作用域受限 worker；固定拓扑、深度 1、非新范式、不改核心循环、opt-in（向导默认开）、关掉零痕迹 |
| **v1+** | [`14-forge-add-incremental-regeneration.md`](./14-forge-add-incremental-regeneration.md)、[`15-llm-robustness-and-context.md`](./15-llm-robustness-and-context.md) | `forge add` 增量再生成 / eval harness / 更多 preset / RAG / 联网 MCP registry / 共享部署的独立认证与控制面隔离 等 |
...
> multi-agent 不是一刀切禁掉——红线是「通用编排框架」。已落地为 `subagents` opt-in 模块（agent-as-tool，固定拓扑、深度 1，生成为自有代码，stdlib 线程池，关闭零痕迹，见 Slice 14），仍**不**做动态图 / DSL / 运行期编排引擎。跨会话长期记忆不在红线之列——它做成薄 + spec 开关 + 不落密钥 + 薄注册表,但不得演变为云托管记忆服务。
