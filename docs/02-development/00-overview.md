...
graph LR
    ...
    S9 --> S10["Slice 10 HITL 确认 ★必做<br/>before_tool 交互确认(allow/reject)"]
    S10 --> S11["Slice 11 MCP 健康/管理<br/>mcp status/doctor + 面板增删/热重连"]
...
| **Slice 10** HITL 交互确认 ★v1 必做(独立做;原与旧 Slice 9 文件级 Checkpoints 配对的"工具护栏对"已因该 Checkpoints 撤销而解除,现 Slice 9 改为停止/继续/重问、与本片各自独立) | (实现前细化) | **工具调用交互式确认**:内置可选 `ConfirmHooks(Hooks)` — 交互式 TTY/Web 弹 **allow / reject / always-allow**(记本会话或写回 `config.yaml`);**非交互 / Web 公开面默认拒绝**;`config.yaml tools.confirm` 选哪些工具需确认;default-off 零痕迹。**复用现有 `before_tool` 挂点**(`Hooks` 已就位,文档已注明 HITL 确认 live here)。让危险工具敢"预置不放行"。复用现成 `before_tool` 挂点 + Web SSE 审批往返管道(原拟与旧 Slice 9 Checkpoints 共建,该 Checkpoints 已撤销、现 Slice 9 改为停止/继续/重问,本片独立搭)。**状态:📋 已登记(人 2026-06-07 定向,取代原 Slice 8+ 内嵌登记;独立做),实现前细化** | allow/reject/always-allow 三态(mock TTY/Web)绿;**非交互默认拒绝断言**;关闭零痕迹;**文档讲清"护栏≠安全边界"**(威胁模型 A;锁能力仍靠生成期不编译,`01 §4`) | ① **是否据此把 shell 默认开 = 改 `01 §6` 全局口径,需人签字**(沿用既有登记);② always-allow 记到会话 vs `config.yaml` 的默认 |
...
不做:生产级权限系统、云托管、**通用多 agent 编排框架 / 工作流编排 DSL / 动态图引擎 / 运行期范式抽象层**、在线 MCP registry、沙箱、**HarnessForge 侧中心化配置/托管**;以及 L3/v1+ 项(RAG / 联网 MCP registry / `/config` 热重载 / keyring / 完整 HITL Web / context offload / **一种 supervisor multi-agent** / 周期预算 / ~~**跨会话全局记忆**~~(非红线,人 2026-06-07 定向上移 v1 必做、紧接会话持久化记为 **Slice 8B**;**✅ 已于 2026-06-07 完成**,见 §2 与 `095-slice-8b-memory.md`)/ **会话持久化·停止/继续/重问·HITL 确认**(已升格为 v1 必做 Slice 8/9/10;**Slice 9 原为文件级 Checkpoints,2026-06-08 撤销后槽位重定义为"停止/继续/重问"会话时间旅行**,见 §2 与 `../03-feature-landscape-and-proposals.md` §3 T1-C)/ **MCP 健康/管理**(升格 Slice 11)/ **原生 Anthropic 双规范 + 推理流式**(升格 Slice 12,§6.4 待签))在 MVP 内不做。(注:**多范式(单 loop Agent/Plan/Ask)已进 Slice 5、工具基线 + 标准 SKILL 进 Slice 6**——它们不是被禁项;被禁的是"通用多 agent 编排框架 / 范式抽象层"。MCP 远程 HTTP/SSE 传输已于 2026-06-03 提前进 Slice 4/L2。)
