...
| Multi-agent 子代理（`subagents.enabled`） | agent-as-tool / orchestrator-worker：主 agent 经 `dispatch` 工具把独立子任务并行委派给作用域受限的 worker（独立上下文 + 各自工具/模型），固定拓扑、深度 1；非新范式、不改核心循环（详见 [`16-slice-14-subagents.md`](./16-slice-14-subagents.md)） |
...
| 范式 / multi-agent | 默认 `agent`（ReAct 式 tool-calling 循环）；生成期多选 Agent/Plan/Ask（对齐 Cursor 三件套）渲染共存薄循环、运行期每轮选一种（CLI `--mode` + Web 下拉；`enabled`+`default` 进 `config.yaml`，首项种默认）；Plan/Ask 只读（只 offer 非高风险工具 + 执行期拒绝集合外调用）。**范式可扩展 = 与 tools 同款薄注册表 + `@register_paradigm`**，运行期用户可自加；内置范式各自自包含、互不 import；注册表始终存在。反思不作独立范式（agent 已在线自纠；Reflexion 作 `AGENTS.md` 用户扩展范例，需真实成功信号）。**multi-agent 已落地为 `subagents` opt-in 模块（agent-as-tool / orchestrator-worker，非新范式）**：supervisor = 默认 `agent` 范式自主调 `dispatch` 工具，把独立子任务**并行**委派给作用域受限 worker（独立上下文 + 各自工具/profile），**固定拓扑、深度 1**、生成为自有代码、关闭零痕迹、零新增依赖（stdlib 线程池），详见 [`16-slice-14-subagents.md`](./16-slice-14-subagents.md)；**仍禁**通用多 agent 编排框架 / 工作流 DSL / 动态图引擎 / 运行期范式抽象层 |
...
| **Slice 14** multi-agent | [`16-slice-14-subagents.md`](./16-slice-14-subagents.md) | subagents（agent-as-tool / orchestrator-worker）：主 agent 经 `dispatch` 工具并行委派给作用域受限 worker；固定拓扑、深度 1、非新范式、不改核心循环、opt-in、关掉零痕迹 |
