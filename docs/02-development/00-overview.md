...
| **Slice 8B** 跨会话记忆 ★v1 必做 | [`095-slice-8b-memory.md`](./095-slice-8b-memory.md) | **跨会话续跑的持久长期记忆/笔记**:agent 写/读一份**少量、自维护**的 markdown 长期笔记(`.harness/memory.md`,**不落密钥**),每轮注入系统提示;薄注册表 `@register_memory`(同 tools/context/budget/paradigms)让用户写自己的记忆后端(向量库/mem0/…),后端契约 `recall`/`record` + 可选 `tools()`/`on_session_end`/`on_compact`,在各生命周期阶段主动管理记忆。内置 `file` 后端默认;工具驱动写入(`memory_read`/`append`/`write`);CLI `memory show/clear/path` + Web `GET/POST/DELETE /memory`。**与 RAG 严格区分**。对标 Hermes 两层记忆的薄简化版(去 Provider 重协议/云托管)。**状态:✅ 已完成(2026-06-07;快测 119 + 产物 test_memory 10/test_web +2 + golden 11 含 docker + memory docker 离线冒烟;ReadLints clean)** | ✅ 记忆写入/读取/跨会话注入(mock)绿;✅ **记忆文件不含密钥断言**;✅ 关闭零痕迹(memory.py/test 不渲染、config/prompts/context/范式/cli/web 逐字一致)、零新增依赖;✅ 可扩展(自定义后端 + 生命周期钩子);✅ 黄金路径绿 + 大改动回归(golden 全量 + Docker) | ① 存储=markdown 笔记 ✅;② 写入=工具驱动 ✅;③ 默认关+关闭零痕迹 ✅;④ 扩展=薄 `@register_memory` ✅;⑤ 后端契约=中档(recall/record+可选 tools/on_session_end/on_compact)✅(人 2026-06-07 选择器签) |
...
不做:生产级权限系统、云托管、**通用多 agent 编排框架 / 工作流编排 DSL / 动态图引擎 / 运行期范式抽象层**、在线 MCP registry、沙箱、**HarnessForge 侧中心化配置/托管**;以及 L3/v1+ 项(RAG / 联网 MCP registry / `/config` 热重载 / keyring / 完整 HITL Web / context offload / **一种 supervisor multi-agent** / 周期预算 / ~~**跨会话全局记忆**~~(非红线,人 2026-06-07 定向上移 v1 必做、紧接会话持久化记为 **Slice 8B**;**✅ 已于 2026-06-07 完成**,见 §2 与 `095-slice-8b-memory.md`)/ **会话持久化·Checkpoints·HITL 确认**(已升格为 v1 必做 Slice 8/9/10)/ **MCP 健康/管理**(升格 Slice 11)/ **原生 Anthropic 双规范 + 推理流式**(升格 Slice 12,§6.4 待签))在 MVP 内不做。(注:**多范式(单 loop Agent/Plan/Ask)已进 Slice 5、工具基线 + 标准 SKILL 进 Slice 6**——它们不是被禁项;被禁的是"通用多 agent 编排框架 / 范式抽象层"。MCP 远程 HTTP/SSE 传输已于 2026-06-03 提前进 Slice 4/L2。)
