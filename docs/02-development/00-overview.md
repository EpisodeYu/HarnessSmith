...
| 技能 | 支持 **Agent Skills 开放标准**（`SKILL.md` + 渐进披露 L1 发现注入 / L2 文件工具或 `read_skill` 读正文 / L3 脚本经工具跑），`spec.skills.enabled` 门控、不引框架；技能脚本 = 高风险默认关。**扩展可发现性**：产物 `GET /registries` + CLI `info` 内省注册表（`PARADIGMS`/`STRATEGIES`/`CONDITIONS`/`memory backends` + 已 import `extensions` 模块 + 已挂 `hooks`），Web 配置页据此渲染下拉/勾选列表、对未注册名字标 ⚠；Web System 页可直接编辑 `extensions`/`hooks`（本地可信面，编辑 `extensions` = 触发本机 import，勿公网暴露 `/config`） |
