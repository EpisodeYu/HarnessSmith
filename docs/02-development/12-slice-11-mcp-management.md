...
>
> **2026-06-10 取数(源+代理)自动解析 + 预热 + 可读报错**:Node 装好后 `desktop-commander` 在企业代理机器上仍 `TimeoutError`——根因是 **npx/npm 不读 Windows 系统代理**、产物启动 npx 子进程只透传 `os.environ` 未注入代理,npx 直连 npm 源被拦、`_npx` 缓存恒空。`mcp.py` 把 `_mirror_env`(单镜像 + TCP 探测)升级为 **`_stdio_net_env`**:`_discover_proxy`(`mcp.proxy` 或 `getproxies()`)+ 在 `[override, 官方, 镜像] × [直连, 走代理]` 候选里用 **HTTP 探测**取首个可达源,注入 `npm_config_registry`/`UV_DEFAULT_INDEX`(+ 命中代理则 `HTTP(S)_PROXY`/`npm_config_proxy`),npm/uv 并发探测、`setdefault` 不覆盖用户值;新增运行期旋钮 `McpConfig.proxy/npm_registry/pip_index`(仅值)。连接失败经 `_connect_error` 翻译为可读建议(换源/设 `mcp.proxy`/预装/调大超时)。新增 **`prewarm_stdio_packages` + CLI `mcp warm`**(fetch-only `npx --package <pkg> -- node -e 0` / `uvx --from <pkg> python -c ""`,opt-in 手动预拉,不进向导 job 以免阻塞产物页打开)。**首连实时进度 + 工具增量**:`status()` 增 `connecting_seconds`/`log_tail`(经 `stdio_client(errlog=临时文件)` 捕获 npx/uvx 拉包 stderr),管理页/Tools 页 amber 时显示"connecting… Ns · <stderr 尾>";`/mcp/discover` 改**非阻塞**(返回 `pending` + per-server `connecting/connected`),Tools 页轮询、**哪个 server 先就绪先显示其工具**,不被慢/失败者卡住。详见 `08-slice-7-wizard.md §2` 同日条目;真机实测 npx 经代理成功拉 DC、`_npx` 缓存填充、golden web+mcp 绿。
>
...
- [x] **单 server 重连 + 连接中三态(2026-06-10 增,Slice 4 ⑧)**:`McpManager.reconnect_server(name)` 只重连一个(其余不动);`status()` 增 `connecting`(绿/黄/红三态)。web `_ensure_mcp_manager` 改非阻塞 + 后台 `wait_ready→sync`,`/mcp/status` 立刻可显示"连接中"(uvx/npx 冷下载不再像卡死);新增 `POST /mcp/servers/{name}/reconnect`;前端每 server 重连按钮 + 黄点(**connecting 时呼吸闪烁 `animate-pulse`,2026-06-10**,慢连不像卡死) + `scheduleMcpPoll` 轮询收敛。覆盖:`test_web.py::test_mcp_reconnect_single_server` / `test_index_has_per_server_reconnect_and_connecting_dot`,`test_mcp_status_reports_live_server_health`(先 connecting 后 connected),`test_mcp.py::test_reconnect_server_restarts_one_in_place` / `test_status_reports_connecting_field` / `test_begin_is_non_blocking_then_wait_ready`。
