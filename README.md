...
- **Combinable budgets + JSONL trace** — per-run stop conditions (`max_steps` / `max_seconds` / `max_tokens` / `max_cost`) combined with `or`/`and`, optional tumbling-window rates, and a user-extensible condition registry; every run writes a JSONL trace with token/cost counts. An opt-in **local-only debug log** (toggle it live on the `/config` Observability page) records the loop/tool/LLM lifecycle as names, counts and durations — never message content, tool arguments, or secrets — and is never uploaded.
