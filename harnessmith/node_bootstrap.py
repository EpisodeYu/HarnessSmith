            ...
            with dest.open("wb") as fh:
                while True:
                    ...
                    if total and got * 10 // total != bucket:  # log roughly every 10%
                        bucket = got * 10 // total
                        _emit(log, f"[harnessmith]   downloading Node ... {got * 100 // total}%")
        return True
    except Exception as exc:  # stall / timeout / HTTP / connection — try the next source
        _emit(log, f"[harnessmith]   source failed: {exc}")
...
def ensure_portable_node(
    ...
    tag = _platform_tag()
    if tag is None:
        _emit(log, f"[harnessmith] portable Node: unsupported platform {platform.platform()!r}; skipping.")
        return None
    ...
    if node_exe.exists():
        _emit(log, f"[harnessmith] portable Node already present: {bin_dir}")
        return str(bin_dir)
    ...
    for base in _sources(prefer_mirror):
        _emit(log, f"[harnessmith] downloading Node {NODE_LTS_VERSION} ({nodeos}-{nodearch}) from {base} ...")
        if not _download(f"{base}/{NODE_LTS_VERSION}/{archive}", tmp, log=log):
            _emit(log, "[harnessmith] trying the next source ...")
            continue
        try:
            _emit(log, "[harnessmith] extracting Node ...")
            _extract(tmp, base_dir, ext)
            tmp.unlink(missing_ok=True)
        except Exception as exc:  # bad archive — non-fatal, stop here
            _emit(log, f"[harnessmith] portable Node extract failed: {exc}")
            return None
        if node_exe.exists():
            _emit(log, f"[harnessmith] Node is ready: {bin_dir}")
            return str(bin_dir)
        _emit(log, "[harnessmith] portable Node: node binary not found after extract.")
        return None
    _emit(log, "[harnessmith] portable Node: all sources failed (see proxy / network).")
    return None
