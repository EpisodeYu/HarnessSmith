...
Every harnessmith invocation (CLI, wizard) appends to
``~/.harnessmith/logs/debug.log`` (rotating at ~1 MB; override the directory
with ``HARNESSMITH_LOG_DIR``). The point is post-hoc diagnosis: when a
...
def _log_dir() -> Path:
    ...
    return Path(override) if override else Path.home() / ".harnessmith" / "logs"
...
def setup() -> Path | None:
    ...
    changed ``HARNESSMITH_LOG_DIR`` (tests) takes effect on the next call.
